...
STATIC BOOLEAN                     mSupportFunction[PSA_MM_FWU_COMMAND_MAX_ID];
  ...
  )
{
  ...
  for (Idx = 0; Idx < FwuInfo->NumFunc; Idx++) {
    mSupportFunction[FunctionPresence[Idx]] = TRUE;
  }

  if ((!mSupportFunction[PSA_MM_FWU_COMMAND_BEGIN_STAGING]) ||
      (!mSupportFunction[PSA_MM_FWU_COMMAND_END_STAGING]) ||
      (!mSupportFunction[PSA_MM_FWU_COMMAND_CANCEL_STAGING]) ||
      (!mSupportFunction[PSA_MM_FWU_COMMAND_OPEN]) ||
      (!mSupportFunction[PSA_MM_FWU_COMMAND_WRITE_STREAM]) ||
      (!mSupportFunction[PSA_MM_FWU_COMMAND_READ_STREAM]) ||
      (!mSupportFunction[PSA_MM_FWU_COMMAND_COMMIT]) ||
      (!mSupportFunction[PSA_MM_FWU_COMMAND_ACCEPT_IMAGE]) ||
      (!mSupportFunction[PSA_MM_FWU_COMMAND_SELECT_PREVIOUS]))
  {
    DEBUG ((DEBUG_ERROR, "Error: Required FWU function(s) not supported.\n"));
    DEBUG ((DEBUG_ERROR, "\tFWU_BEGIN_STATING: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_BEGIN_STAGING] ? "Supported" : "Unsupported"));
    DEBUG ((DEBUG_ERROR, "\tFWU_END_STATING: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_END_STAGING] ? "Supported" : "Unsupported"));
    DEBUG ((DEBUG_ERROR, "\tFWU_CANCEL_STATING: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_CANCEL_STAGING] ? "Supported" : "Unsupported"));
    DEBUG ((DEBUG_ERROR, "\tFWU_OPEN: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_OPEN] ? "Supported" : "Unsupported"));
    DEBUG ((DEBUG_ERROR, "\tFWU_WRITE_STREAM: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_WRITE_STREAM] ? "Supported" : "Unsupported"));
    DEBUG ((DEBUG_ERROR, "\tFWU_READ_STREAM: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_READ_STREAM] ? "Supported" : "Unsupported"));
    DEBUG ((DEBUG_ERROR, "\tFWU_COMMIT: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_COMMIT] ? "Supported" : "Unsupported"));
    DEBUG ((DEBUG_ERROR, "\tFWU_ACCEPT_IMAGE: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_ACCEPT_IMAGE] ? "Supported" : "Unsupported"));
    DEBUG ((DEBUG_ERROR, "\tFWU_SELECT_PREVIOUS: %s\n", mSupportFunction[PSA_MM_FWU_COMMAND_SELECT_PREVIOUS] ? "Supported" : "Unsupported"));
    return EFI_UNSUPPORTED;
  }
  ...
  )
{
  ...
  Status = FwuOpen (&gPsaFwuImageDirectoryGuid, FWU_STREAM_READ, &Handle);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
  ...
  )
{
  ...
  {
    DEBUG ((DEBUG_ERROR, "FmpPsaFwuLib: Not renewed Image Directory after accept - %g\n", &gEfiCallerIdGuid));
  }
}
  ...
  )
{
  ...
  *LastAttemptStatus = LAST_ATTEMPT_STATUS_DEVICE_LIBRARY_MIN_ERROR_CODE_VALUE;
  ...
  Status = FwuOpen (&gEfiCallerIdGuid, FWU_STREAM_WRITE, &Handle);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
  ...
  *LastAttemptStatus = LAST_ATTEMPT_STATUS_SUCCESS;
  ...
  /**
   * Renewal CorrectBoot value on ImageDirectory.
   */
  Status = GetFwuImageDirectory ();
  ...
  /**
   * After end staging, active idx is changed.
   * In other word, the UpdateAgent's status out of correct boot.
   * Therfore, Disable update.
   */
...
CancelUpdate:
  if (mSupportFunction[PSA_MM_FWU_COMMAND_CANCEL_STAGING]) {
  ...
  )
{
  ...
  if (!mImageDirectory->CorrectBoot) {
    ...
    /**
     * Though fwu_select_previous failed,
     * Firmware can run with image in previous image disabling
  ...
  if (mImageEntry->Accepted == FWU_IMAGE_UNACCEPTED) {
    /**
     * When UEFI boot and load this Fmp device driver,
     * Consider the Boot is successful and Accept the Image
     * at ReadyToBoot phase.
     */
    Status = gBS->CreateEventEx (
                    EVT_NOTIFY_SIGNAL,
                    TPL_CALLBACK,
