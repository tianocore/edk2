...
/*
 * Firmware update request mode
 */
typedef enum {
  FWU_REQ_MODE_MM_PROTOCOL,
  FWU_REQ_MODE_FFA_DIRECT_REQ2,
  FWU_REQ_MODE_UNKNOWN,
} FWU_REQ_MODE;
...
STATIC FWU_REQ_MODE                    mFwuReqMode        = FWU_REQ_MODE_UNKNOWN;
STATIC EFI_MM_COMMUNICATION2_PROTOCOL  *mMmCommunication2 = NULL;
STATIC EFI_MM_COMMUNICATE_HEADER       *mPsaFwuCommBuffer = NULL;
...
{
  if (mFwuReqMode == FWU_REQ_MODE_MM_PROTOCOL) {
    gRT->ConvertPointer (0x00, (VOID **)&mMmCommunication2);
    gRT->ConvertPointer (0x00, (VOID **)&mPsaFwuCommBuffer);
  }
}
  ...
  )
{
  ...
  if ((RespMsg == NULL) ||
      ((mFwuReqMode == FWU_REQ_MODE_MM_PROTOCOL) &&
       ((mMmCommunication2 == NULL) || (mPsaFwuCommBuffer == NULL))) ||
      ((mFwuReqMode == FWU_REQ_MODE_FFA_DIRECT_REQ2) && (mPsaFwuBuffer == NULL)))
  {
  ...
  *RespMsg     = NULL;
  ...
  TotalBufferSize += (mFwuReqMode == FWU_REQ_MODE_MM_PROTOCOL) ?
                     sizeof (EFI_MM_COMMUNICATE_HEADER) : 0;

  if (mFwuReqMode == FWU_REQ_MODE_MM_PROTOCOL) {
    ZeroMem (mPsaFwuCommBuffer, TotalBufferSize);
    CopyGuid (&mPsaFwuCommBuffer->HeaderGuid, &gPsaFwuUpdateAgentGuid);
    mPsaFwuCommBuffer->MessageLength = MAX (TotalReqCommBufferSize, TotalRespCommBufferSize);
    CmdData                          = (PSA_MM_FWU_CMD_DATA *)&mPsaFwuCommBuffer->Data;
  } else {
  ...
  if (mFwuReqMode == FWU_REQ_MODE_MM_PROTOCOL) {
    *RespMsg = (PSA_MM_FWU_CMD_DATA *)&mPsaFwuCommBuffer->Data;
  } else {
...
/**
 * Check whether firmware update uses via Mm communication or not.
 *
 * @param [out]   FwuReqMode              Firmware request mode
 * @param [out]   UpdateAgentId           if FWU_REQ_MODE_FFA_DIRECT_REQ2,
 *                                        update agent partition id
 *
 * @retval FWU_REQ_MODE_MM_PROTOCOL       Use mm communication protocol
 * @retval FWU_REQ_MODE_FFA_DIRECT_REQ2   Use ff-a direct message req v2
 *
 */
STATIC
EFI_STATUS
  ...
  )
{
  ...
  if (!IsFfaSupported ()) {
    *FwuReqMode = FWU_REQ_MODE_MM_PROTOCOL;
    return EFI_SUCCESS;
  }
  ...
  if (EFI_ERROR (Status)) {
    ...
    *FwuReqMode = FWU_REQ_MODE_UNKNOWN;
    return Status;
  }
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a: Failed to get Tx/Rx Buffer. Status:%r\n",
      __func__,
      Status
      ));
    *FwuReqMode = FWU_REQ_MODE_UNKNOWN;
  ...
  *FwuReqMode    = FWU_REQ_MODE_FFA_DIRECT_REQ2;
  ...
  )
{
  ...
  if ((UpdateCount < 0) || ((UpdateCount > 0) && (UpdateGuids == NULL))) {
    return EFI_INVALID_PARAMETER;
  }
 ...
 * @param [in]   OpType             FWU_STREAM_READ or FWU_STREAM_WRITE.
...
/**
  Clear resources alloacted by PsaFwuLibInit().
  ...
  )
{
  ...
  mMmCommunication2 = NULL;

  if (mPsaFwuCommBuffer != NULL) {
    FreePool (mPsaFwuCommBuffer);
    mPsaFwuCommBuffer = NULL;
  }
}
  ...
  )
{
  ...
  Status = FwuGetRequestMode (&mFwuReqMode, &mUpdateAgentId);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  if (mFwuReqMode == FWU_REQ_MODE_MM_PROTOCOL) {
    mPsaFwuCommBuffer = AllocateRuntimePool (MM_SHARED_BUFFER_SIZE);
    if (mPsaFwuCommBuffer == NULL) {
      return EFI_OUT_OF_RESOURCES;
    }

    // See the boundary check logic on CheckBuffer in StandaloneMmCpu.
    mPsaFwuMaxBufferSize = MM_SHARED_BUFFER_SIZE - sizeof (EFI_MM_COMMUNICATE_HEADER);
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_INFO,
        "%a: Failed to load Mm communication protocol (%r)\n",
        __func__,
        Status
        ));
      goto ErrorHandler;
  ...
  DEBUG ((DEBUG_INFO, "%a: mFwuReqMode: %d\n", __func__, mFwuReqMode));
