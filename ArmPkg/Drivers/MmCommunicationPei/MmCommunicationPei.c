  ...
  )
{
  ...
  if ((StmmPartInfo->PartitionProps & FFA_PART_PROP_RECV_DIRECT_REQ) == 0x00) {
    Status = EFI_UNSUPPORTED;
    DEBUG ((DEBUG_ERROR, "StandaloneMm doesn't support DIRECT_MSG_RESP...\n"));
    goto ErrorHandler;
  }
  ...
  )
{
  ...
  if (IsFfaSupported ()) {
    Status = SendFfaProtoMmCommunicate ();
  } else {
    Status = SendMmProtoMmCommunicate ();
  }
