  ...
  )
{
  ...
  while (TRUE) {
    ...
    if ((TimeoutInMicroseconds != 0) && (Timeout == 0)) {
      DEBUG ((DEBUG_ERROR, "StartupThisAP: Timeout!\n"));
      return EFI_TIMEOUT;
    }
  ...
  )
{
  ...
  if (!Found) {
    DEBUG ((
      DEBUG_ERROR,
      "Adding CPU %d to failed (FLI=%llu)\n",
      ProcessorIndex,
      mCpuMpData.FailedListIndex
      ));
  ...
  )
{
  ...
  for (Index = 0; Index < mCpuMpData.NumberOfProcessors; Index++) {
    ...
    // If a CPU is in the Finished state (i.e. the AP finished its work but
    // the BSP didn't get around to resetting it to the Idle state), reset
    // it back to Idle.
    if (GetApState (CpuData) == CpuStateFinished) {
      CpuData->State = CpuStateIdle;
    }
