  ...
  )
{
  ...
  for (Index = 0; Index < mCpuMpData.NumberOfProcessors; Index++) {
    if (mCpuMpData.CpuData[Index].State == CpuStateFinished) {
      mCpuMpData.CpuData[Index].State = CpuStateIdle;
    }
  }
}
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "DispatchCpu returned not ready\n"));
