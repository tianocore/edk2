  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a: STE promotion failed for SmmuBase=0x%llx alias StreamId 0x%x: %r\n",
      __func__,
      SmmuInfo->SmmuBase,
      AliasStreamId,
      Status
...
/**
  ...
  @param [in]  Attributes                 Attributes to set for the mapping. Must be a valid Stage 2 Translation Table attribute (12 bit or less).
