  ...
  )
{
  ...
  if (SerialPortInfo->PortSubtype == EFI_ACPI_DBG2_PORT_SUBTYPE_SERIAL_FULL_16550) {
  ...
  } else if (SerialPortInfo->PortSubtype == EFI_ACPI_DBG2_PORT_SUBTYPE_SERIAL_16550_WITH_GAS) {
    ...
    if ((NonBsaHid != NULL) && (AsciiStrLen (NonBsaHid) != 0)) {
      ...
      DEBUG ((
        DEBUG_ERROR,
        "ERROR: SSDT-SERIAL-PORT-FIXUP: PcdNonBsaCompliant16550SerialHid not set.\n"
        ));
    ...
    DEBUG ((
      DEBUG_ERROR,
      "ERROR: SSDT-SERIAL-PORT-FIXUP: Invalid Serial Port Subtype %d.\n",
      SerialPortInfo->PortSubtype
      ));
    Status = EFI_INVALID_PARAMETER;
    goto exit_handler;
  ...
  if (SerialPortInfo->PortSubtype == EFI_ACPI_DBG2_PORT_SUBTYPE_SERIAL_FULL_16550) {
    ...
    Status = AmlCodeGenRdUartSerialBus (
               ...
               0x1,                                   // TxFifo
               0,                                     // Parity
               0,                                     // SerialLinesEnabled
               NULL,                                  // VendorDefinedData
               0,                                     // VendorDefinedDataLength
               NULL,                                  // Additional vendor data
               0,                                     // Additional vendor data length
