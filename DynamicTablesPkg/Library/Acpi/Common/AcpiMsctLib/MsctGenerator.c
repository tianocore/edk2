  ...
  )
{
  ...
  UINT32      ProximityDomainId;
  UINT32      Index;

  EFI_ACPI_6_5_MAXIMUM_PROXIMITY_DOMAIN_INFORMATION_STRUCTURE  *ProcDomain;
  EFI_ACPI_6_5_MAXIMUM_PROXIMITY_DOMAIN_INFORMATION_STRUCTURE  *ProcDomainRaw;
  ...
  ProcDomainRaw = AllocateZeroPool (
                    sizeof (EFI_ACPI_6_5_MAXIMUM_PROXIMITY_DOMAIN_INFORMATION_STRUCTURE) *
                    ProcDomainArchCount
                    );
  if (ProcDomainRaw == NULL) {
    Status = EFI_OUT_OF_RESOURCES;
    goto error_handler;
  }

  ///
  /// ProcDomainArch contains the proximity domain IDs.
  /// Group them into similar domain IDs.
  /// Also find the maximum number of unique proximity domain IDs.
  ///
  MaxProcDomain = 0;
  for (Index = 0; Index < ProcDomainArchCount; Index++) {
    ProximityDomainId = ProcDomainArch[Index];

    ProcDomainRaw[ProximityDomainId].Revision = EFI_ACPI_6_5_MAXIMUM_SYSTEM_CHARACTERISTICS_TABLE_REVISION;
    ProcDomainRaw[ProximityDomainId].Length   = sizeof (EFI_ACPI_6_5_MAXIMUM_PROXIMITY_DOMAIN_INFORMATION_STRUCTURE);

    ProcDomainRaw[ProximityDomainId].ProximityDomainRangeLow  = ProximityDomainId;
    ProcDomainRaw[ProximityDomainId].ProximityDomainRangeHigh = ProximityDomainId;
    ProcDomainRaw[ProximityDomainId].MaximumProcessorCapacity++;

    MaxProcDomain = MAX (MaxProcDomain, ProximityDomainId);
  }

  /// Adjust zero based proximity domain
  MaxProcDomain++;
  ...
  CopyMem (
    ProcDomain,
    ProcDomainRaw,
    MaxProcDomain * sizeof (EFI_ACPI_6_5_MAXIMUM_PROXIMITY_DOMAIN_INFORMATION_STRUCTURE)
    );

  *ProcDomainInfo      = ProcDomain;
  *ProcDomainInfoCount = MaxProcDomain;
  FreePool (ProcDomainArch);
  FreePool (ProcDomainRaw);
  return EFI_SUCCESS;
  ...
  )
{
  ...
  for (MemIndex = 0; MemIndex <= MemDomainInfoCount; MemIndex++) {
    ...
    MaxProximityDomain = MAX (
                           MaxProximityDomain,
                           MemDomainInfo[MemIndex].ProximityDomainRangeHigh
                           );
  ...
  /// Club the symmetric continuous processor proximity domains
  PerformQuickSort (
    ProcDomainInfo,
    ProcDomainInfoCount,
    sizeof (EFI_ACPI_6_5_MAXIMUM_PROXIMITY_DOMAIN_INFORMATION_STRUCTURE),
