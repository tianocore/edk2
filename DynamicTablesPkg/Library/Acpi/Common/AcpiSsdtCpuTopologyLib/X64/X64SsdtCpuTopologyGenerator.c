...
{
  if ((AslName == NULL) || (AslNameSize < (AML_NAME_SEG_SIZE + 1))) {
    return EFI_INVALID_PARAMETER;
  }

  CopyMem (AslName, CSTS_SCOPE, AslNameSize);

  AslName[3] = AsciiFromHex ((Index >> 4) & 0xF);

  if (Index > 0xFF) {
    AslName[2] = AsciiFromHex ((Index >> 8) & 0xF);
  }

  if (Index > 0xFFF) {
    AslName[1] = AsciiFromHex ((Index >> 12) & 0xF);
  }
  ...
  )
{
  ...
  LastIndex = CstTokenTable->LastIndex;
  for (Index = 0; Index < LastIndex; Index++) {
    Status = GenerateCstAslName (AslName, AML_NAME_SEG_SIZE + 1, LastIndex);
    if (EFI_ERROR (Status)) {
  ...
  )
{
  ...
  Status = GenerateCstAslName (AslName, AML_NAME_SEG_SIZE + 1, CstTokenTable->LastIndex);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    return Status;
  }
