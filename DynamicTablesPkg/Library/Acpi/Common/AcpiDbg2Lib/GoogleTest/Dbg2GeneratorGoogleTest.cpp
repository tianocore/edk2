...
protected:
    ...
    )
  {
    ...
    for (UINT32 i = 0; i < Count; i++) {
      ...
      info.BaseAddress       = 0x1000 * (i + 1);
      info.BaseAddressLength = 0x1000;
    ...
    EXPECT_CALL (MockConfigMgrProtocol, GetObject (_, CREATE_CM_ARCH_COMMON_OBJECT_ID (EArchCommonObjDbg2DeviceInfo), CM_NULL_TOKEN, _))
    ...
    )
  {
    ...
    for (UINT32 i = 0; i < 2; i++) {
      ...
      desc.BaseAddress = 0x1000 + (i * 0x1000);
      desc.Length      = 0x1000;
    ...
    EXPECT_CALL (MockConfigMgrProtocol, GetObject (_, CREATE_CM_ARCH_COMMON_OBJECT_ID (EArchCommonObjDbg2DeviceInfo), CM_NULL_TOKEN, _))
      .WillOnce (
         DoAll (
           SetArgPointee<3>(
             CM_OBJ_DESCRIPTOR {
      CREATE_CM_ARCH_COMMON_OBJECT_ID (EArchCommonObjDbg2DeviceInfo),
...
TEST_F (Dbg2GeneratorTest, BuildDbg2TableEx_SingleSerialPort) {
  ...
  ValidateSerialDeviceInfo (DeviceInfo, 0x1000ULL, 0x1000U);
    ...
    ) override
  {
    ...
    for (UINT32 i = 0; i < config.DeviceCount; i++) {
      ...
      for (UINT32 j = 0; j < config.RangesPerDevice; j++) {
        ...
        range.BaseAddress = 0x1000 + (totalRanges + j) * 0x1000;
        range.Length      = 0x1000;
...
TEST_F (Dbg2GeneratorAdversarialTest, TooManyMemoryRanges) {
  ...
  for (UINT32 i = 0; i <= MAX_UINT8 + 1; i++) {
    CM_ARCH_COMMON_MEMORY_RANGE_DESCRIPTOR  range = { 0 };
    range.BaseAddress = 0x1000 + (i * 0x1000);
    range.Length      = 0x1000;
...
TEST_F (Dbg2GeneratorAdversarialTest, InvalidSerialPortAccessSize) {
  ...
  serialPort.BaseAddress       = 0x1000;
  serialPort.BaseAddressLength = 0x1000;
...
TEST_F (Dbg2GeneratorTest, MultipleSerialPorts) {
  ...
  port1.BaseAddress       = 0x1000;
  port1.BaseAddressLength = 0x1000;
  ...
  port2.BaseAddress       = 0x2000;
  port2.BaseAddressLength = 0x1000;
