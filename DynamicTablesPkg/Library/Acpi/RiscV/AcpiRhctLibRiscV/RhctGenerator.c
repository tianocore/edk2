  ...
  )
{
  ...
  while (NodeCount-- != 0) {
    ...
    Size += (UINT32)(sizeof (EFI_ACPI_6_6_RHCT_ISA_STRING_NODE)) +
            ALIGN_VALUE ((AsciiStrSize (NodeList->IsaString) + 1), 2);
    (*NodeIndexer)++;
    NodeList++;
  }
  ...
  )
{
  ...
  while (NodeCount-- != 0) {
    IsaLength                = AsciiStrSize (NodeList->IsaString) + 1;
  ...
  )
{
  ...
  while (NodeCount-- != 0) {
    ...
    Offsets = AllocateZeroPool (sizeof (UINT32) * NumOffsets);
    Idx     = 0;
  ...
  )
{
  ...
  *Table = (EFI_ACPI_DESCRIPTION_HEADER *)AllocateZeroPool (TableSize);
  ...
  Rhct->Flags        = TimerInfo->Flags ? EFI_ACPI_6_6_RHCT_FLAG_TIMER_CANNOT_WAKEUP_CPU : 0;
