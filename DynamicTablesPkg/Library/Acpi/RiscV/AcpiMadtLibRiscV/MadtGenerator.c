  ...
  )
{
  ...
  ASSERT ((RintcInfo->Flags & ~(UINT32)(EFI_ACPI_6_6_RINTC_FLAG_ENABLE)) == 0);
...
/**
  Function to test if two RINTC information structures have the
  ...
  @param [in]  Index1             Index of RintcInfo1 in the shared list of RINTC
                                  Info structures.
  @param [in]  Index2             Index of RintcInfo2 in the shared list of RINTC
                                  Info structures.

  @retval TRUE                    RintcInfo1 and RintcInfo2 have the same UID.
  @retval FALSE                   RintcInfo1 and RintcInfo2 have different UIDs.
**/
BOOLEAN
EFIAPI
IsAcpiUidEqual (
  ...
  )
{
  ...
  // UINT16 NumIds
  ASSERT (
    ((ImsicInfo->NumIds >= IMSIC_MIN_NUM_IDS) &&
     (ImsicInfo->NumIds <= IMSIC_MAX_NUM_IDS) &&
     ((ImsicInfo->NumIds + 1) % 64 == 0))
    );
  ...
  ASSERT (
    ImsicInfo->GuestIndexBits >= IMSIC_MIN_GUEST_INDEX_BITS &&
    ImsicInfo->GuestIndexBits <= IMSIC_MAX_GUEST_INDEX_BITS
    );
  Imsic->GuestIndexBits = ImsicInfo->GuestIndexBits;

  // UINT8 HartIndexBits
  ASSERT (
    ImsicInfo->HartIndexBits >= IMSIC_MIN_HART_INDEX_BITS &&
    ImsicInfo->HartIndexBits <= IMSIC_MAX_HART_INDEX_BITS
    );
  Imsic->HartIndexBits = ImsicInfo->HartIndexBits;

  // UINT8 GroupIndexBits
  ASSERT (
    ImsicInfo->GroupIndexBits >= IMSIC_MIN_GROUP_INDEX_BITS &&
    ImsicInfo->GroupIndexBits <= IMSIC_MAX_GROUP_INDEX_BITS
    );
  Imsic->GroupIndexBits = ImsicInfo->GroupIndexBits;

  // UINT8 GroupIndexShift
  ASSERT (
    ImsicInfo->GroupIndexShift >= IMSIC_MIN_GROUP_INDEX_SHIFT &&
    ImsicInfo->GroupIndexShift <= IMSIC_MAX_GROUP_INDEX_SHIFT
    );
  ...
  )
{
  ...
  CopyMem (&Aplic->HwId, &AplicInfo->PlicAplicCommonInfo.Hid, RISCV_HWID_LENGTH);
  ASSERT (IsValidAcpiId ((CHAR8 *)Aplic->HwId) != 0);
  ...
  )
{
  ...
  CopyMem (&Plic->HwId, &PlicInfo->PlicAplicCommonInfo.Hid, RISCV_HWID_LENGTH);
  ASSERT (IsValidAcpiId ((CHAR8 *)Plic->HwId) != 0);
