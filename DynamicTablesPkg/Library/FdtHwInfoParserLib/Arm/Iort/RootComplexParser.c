  ...
  )
{
  ...
  if ((Data == NULL) || (DataSize != (4 * sizeof (UINT32)))) {
    // If error or not FdtMaxTimerItem interrupts.
    ASSERT ((Data != NULL) && (DataSize == (4 * sizeof (UINT32))));
    return EFI_ABORTED;
  }

  ZeroMem (&IdMapping, sizeof (CM_ARM_ID_MAPPING));

  IdMapping.InputBase            = Fdt32ToCpu (((UINT32 *)Data)[0]);
  IdMapping.NumIds               = Fdt32ToCpu (((UINT32 *)Data)[3]);
  IdMapping.OutputBase           = Fdt32ToCpu (((UINT32 *)Data)[2]);
  IdMapping.OutputReferenceToken = CM_ABSTRACT_TOKEN_MAKE (ETokenNameSpaceFdtHwInfo, EFdtHwInfoIortObject, Fdt32ToCpu (((UINT32 *)Data)[1]));
  ...
  RootComplexInfo->IdMappingCount = 1;
