  ...
  )
{
  ...
  *TopLevelSockets = FALSE;
  ...
  FdtForEachSubnode (ChildNode, Fdt, CpuMapNode) {
    SawChild = TRUE;
    if (!FdtNodeHasNameEx (Fdt, ChildNode, "socket")) {
      return EFI_SUCCESS;
    }
  }
  ...
  )
{
  ...
  if ((Context->CpuCount + (TOPOLOGY_CPUS_PER_INDEX - 1)) / TOPOLOGY_CPUS_PER_INDEX > TOPOLOGY_MASK_SIZE) {
    ASSERT (FALSE);
    return EFI_ABORTED;
  }
  ...
  )
{
  ...
  ASSERT ((NodeIndex & ~CM_ABSTRACT_TOKEN_ID_MASK) == 0);
  Node->Token = (CM_OBJECT_TOKEN)CM_ABSTRACT_TOKEN_MAKE (
  ...
  )
{
  ...
  Status = AppendProcHierarchyNode (
             ...
             ParentIndex,
             ParentToken,
             (CM_OBJECT_TOKEN)CM_ABSTRACT_TOKEN_MAKE (ETokenNameSpaceFdtHwInfo, EFdtHwInfoGicCObject, (UINT32)CpuNode),
             Flags,
             &ProcHierarchyNode,
  ...
  )
{
  ...
  if (CpuProp != NULL) {
    ...
    return AddCpuToProcHierarchy (
             Context,
             ParentIndex,
             ParentToken,
             ChildNode,
             FdtNodeHasNameEx (Fdt, Node, "thread")
             );
  }
  ...
  Flags = ((ParentIndex == TOPOLOGY_INVALID_INDEX) &&
           FdtNodeHasNameEx (Fdt, Node, "socket")) ?
