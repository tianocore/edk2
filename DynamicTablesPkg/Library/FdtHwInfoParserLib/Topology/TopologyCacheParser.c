...
//
// Maximum cache level. (Safety check)
//
...
/** Parse a shared DT cache node and its next-level-cache chain.
  ...
  @param [in]      DefaultLevel Default cache level if the DT omits "cache-level".
                                Must have 2 <= level <= 3.
  ...
  )
{
  ...
  if ((Context == NULL) || (Fdt == NULL) || (CacheToken == NULL) ||
      (DefaultLevel < 2) || (DefaultLevel > MAX_CACHE_LEVEL))
  {
    ASSERT (FALSE);
    return EFI_INVALID_PARAMETER;
  }
  ...
  //
  // If 2 CPUs don't share their L2 cache but share L3,
  // L3 should have already been parsed during at the second parsing phase.
  //
  CacheIndex = FindCacheByDtNode (Context, CacheNode);
  if (CacheIndex != TOPOLOGY_INVALID_INDEX) {
    // Cache levels must increase as we only have unified caches.
    if ((Context->CacheBuffers[CacheIndex].Info.Level + 1) < DefaultLevel) {
      ASSERT (FALSE);
      return EFI_INVALID_PARAMETER;
    }
    ...
    if (NextLevelProp != NULL) {
      ...
      //
      // Cf. CacheInfo->Level=0,1,2 is used for level 1,2,3 cache,
      // and ParseCacheNode() uses level 1,2,3 values. Thus do:
      // + 1 (for the 1-based shift)
      // + 1 (next expected cache level is higher)
      //
  ...
  if (CacheProp != NULL) {
    ...
    if ((CacheLevel < 2) || (CacheLevel > MAX_CACHE_LEVEL)) {
      ASSERT (FALSE);
      return EFI_ABORTED;
    }

    // Cache levels must increase as we only have unified caches.
    if (CacheLevel < DefaultLevel) {
      ASSERT (FALSE);
      return EFI_INVALID_PARAMETER;
    }

    // Cf. CacheInfo->Level=0,1,2 is used for level 1,2,3 cache.
    CacheInfo->Level = CacheLevel - 1;
  } else {
    CacheInfo->Level = DefaultLevel - 1;
  }
...
/** Resolve the best processor hierarchy node to own a cache.
  ...
  Select the deepest node whose CPU set exactly matches the cache sharing
  set. Abort if the processor hierarchy cannot represent that sharing set
  exactly.
  ...
  @retval EFI_SUCCESS             The function completed successfully.
  @retval EFI_INVALID_PARAMETER   Invalid parameter.
  @retval EFI_ABORTED             No processor hierarchy node has the exact CPU set.
**/
STATIC
EFI_STATUS
EFIAPI
  ...
  )
{
  ...
  for (Index = 0; Index < Context->ProcHierarchyCount; Index++) {
    ...
    //
    // Associate this cache to the most restrictive ProcHierarchy node.
    // I.e. if the cluster and socket ProcHierarchy node have matching
    // cpumasks, associate the cache to the cluster.
  ...
  //
  // If there is no cpu-map and caches are described, or if one level is missing
  // in the cpu-map (e.g. no cluster level), the CacheCpuMask might not find a perfectly
  // matching ProcHierarchy node mask.
  // Stay strict for now and don't support this case.
  //
  DEBUG ((DEBUG_ERROR, "Topology-parser: Could not associate a cache level to ProcHierarchy cpumask.\n"));
  ASSERT (FALSE);
  return EFI_ABORTED;
}
...
/** Attach parsed caches to their owning processor hierarchy nodes.
  ...
  @retval EFI_SUCCESS             The function completed successfully.
  @retval EFI_INVALID_PARAMETER   Invalid parameter.
  @retval EFI_ABORTED             A cache could not be placed in the topology.
  @retval EFI_OUT_OF_RESOURCES    Memory allocation failed.
**/
STATIC
EFI_STATUS
  ...
  )
{
  EFI_STATUS                          Status;
  UINT32                              CacheIndex;
  UINT32                              NextCacheIndex;
  UINT32                              ProcIndex;
  UINT32                              TotalRefCount;
  UINT32                              *ProcPrivateResCount;
  UINT32                              *PerProcOffset;
  BOOLEAN                             *CacheIsReferenced;
  CM_ARCH_COMMON_CACHE_INFO           *CacheInfo;
  ...
  ProcPrivateResCount = NULL;
  PerProcOffset       = NULL;
  CacheIsReferenced   = NULL;
  RefArray            = NULL;
  ...
  // Number of private resources required for each ProcHierarchy node.
  ProcPrivateResCount = AllocateZeroPool (sizeof (*ProcPrivateResCount) * Context->ProcHierarchyCount);
  // For each ProcHierarchy node, offset to write to in RefArray.
  // RefArray will contain references to the Private Resources owned by a ProcHierarchy node.
  PerProcOffset = AllocateZeroPool (sizeof (*PerProcOffset) * Context->ProcHierarchyCount);
  // TRUE is the cache is a parent cache, private to a ProcHierarchy node.
  CacheIsReferenced = AllocateZeroPool (sizeof (*CacheIsReferenced) * Context->CacheCount);
  if ((ProcPrivateResCount == NULL) || (PerProcOffset == NULL) || (CacheIsReferenced == NULL)) {
  ...
  //
  // ACPI PPTT requires caches to be associated to specific ProcHierarchy nodes.
  // Cf. 5.2.31.2 Cache Type Structure - Type 1: "Next Level of Cache":
  // Reference to next level of cache that is private to the processor topology
  // instance.
  //
  // This means that in the following topology (CPU0/1 have private L1I, L1D and L2):
  // Cluster -------- L3 (shared)
  // |-- CPU 0 ------ L2
  // |               |-- L1D
  // |               +-- L1I
  ...
  //                 |-- L1D
  //                 +-- L1I
  // L2 must be detached from L3, i.e. the parent (resp. "Next Level of Cache") of
  // L2 must be set to NULL. Indeed, L3 is not private to CPU0 or CPU1.
  //
  for (CacheIndex = 0; CacheIndex < Context->CacheCount; CacheIndex++) {
    CacheInfo = &Context->CacheBuffers[CacheIndex].Info;
    if (CacheInfo->NextLevelOfCacheToken == CM_NULL_TOKEN) {
      continue;
    }

    // Find the cache parent.
    for (NextCacheIndex = 0; NextCacheIndex < Context->CacheCount; NextCacheIndex++) {
      if (Context->CacheBuffers[NextCacheIndex].Info.Token == CacheInfo->NextLevelOfCacheToken) {
        break;
      }
    }

    if (NextCacheIndex == Context->CacheCount) {
      ASSERT (FALSE);
      Status = EFI_NOT_FOUND;
      goto exit_handler;
    }

    if (Context->CacheBuffers[CacheIndex].OwnerProcIndex !=
        Context->CacheBuffers[NextCacheIndex].OwnerProcIndex)
    {
      // If the current cache and its parent cache don't belong to the same
      // ProcHierarchy node, break the parent-child relation.
      CacheInfo->NextLevelOfCacheToken = CM_NULL_TOKEN;
    } else {
      // The parent cache is indeed a parent of the ProcHierarchy node,
      // both being private resources.
      CacheIsReferenced[NextCacheIndex] = TRUE;
    }
  }

  //
  // Compute the number of Private Resources each ProcHierarchy requires.
  // Private resources that are referenced by other private resources don't count.
  //
  TotalRefCount = 0;
  for (CacheIndex = 0; CacheIndex < Context->CacheCount; CacheIndex++) {
    // Only consider the head of a private resource list.
    if (CacheIsReferenced[CacheIndex]) {
      continue;
    }

    ProcIndex = Context->CacheBuffers[CacheIndex].OwnerProcIndex;
    ProcPrivateResCount[ProcIndex]++;
    TotalRefCount++;
  }
  ...
  //
  // Init. the offset at which a ProcHierarchy will write in the RefArray.
  //
  for (ProcIndex = 1; ProcIndex < Context->ProcHierarchyCount; ProcIndex++) {
    PerProcOffset[ProcIndex] = PerProcOffset[ProcIndex - 1] + ProcPrivateResCount[ProcIndex - 1];
  }

  //
  // Populate RefArray. Each ProcHierarchy node writes at PerProcOffset[ProcIndex] offset.
  //
  for (CacheIndex = 0; CacheIndex < Context->CacheCount; CacheIndex++) {
    // Only consider the head of a private resource list.
    if (CacheIsReferenced[CacheIndex]) {
  ...
  //
  // For each ProcHierarchy node, store an array referencing all the Private Resources
  // this ProcHierarchy owns.
  //
  for (ProcIndex = 0; ProcIndex < Context->ProcHierarchyCount; ProcIndex++) {
    if (ProcPrivateResCount[ProcIndex] == 0) {
      continue;
    }

    // Retrieve the start of the Private Resources elements for this ProcHierarchy node.
    PerProcOffset[ProcIndex] -= ProcPrivateResCount[ProcIndex];

    ProcHierarchyNode                       = &Context->ProcHierarchyBuffers[ProcIndex].Info;
    ProcHierarchyNode->NoOfPrivateResources = ProcPrivateResCount[ProcIndex];
    Status                                  = AddSingleCmObjArray (
                                                ...
                                                sizeof (CM_ARCH_COMMON_OBJ_REF) * ProcPrivateResCount[ProcIndex],
                                                ProcPrivateResCount[ProcIndex],
                                                &ProcHierarchyNode->PrivateResourcesArrayToken
                                                );
    if (EFI_ERROR (Status)) {
      ASSERT (FALSE);
      goto exit_handler;
...
exit_handler:
  if (CacheIsReferenced != NULL) {
    FreePool (CacheIsReferenced);
  }

  if (ProcPrivateResCount != NULL) {
    FreePool (ProcPrivateResCount);
  }
