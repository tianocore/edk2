  ...
  )
{
  ...
  *Table = NULL;
  ...
  // validate that BiosSize and ExtendedBiosSize are correctly configured
  if (((BiosInfo->BiosSize != 0xFF) &&
       (BiosInfo->ExtendedBiosSize.Size == 0)) ||
      ((BiosInfo->BiosSize == 0xFF) &&
       (BiosInfo->ExtendedBiosSize.Size != 0)))
  {
    SmbiosRecord->BiosSize = BiosInfo->BiosSize;
  } else {
    DEBUG ((DEBUG_ERROR, "%a: BiosSize and/or ExtendedBiosSize are invalid\n", __func__));
  ...
  // make sure the Unit field does not have reserved values
  if (BiosInfo->ExtendedBiosSize.Unit <= 0b01) {
    SmbiosRecord->ExtendedBiosSize = BiosInfo->ExtendedBiosSize;
  } else {
    DEBUG ((DEBUG_ERROR, "%a: ExtendedBiosSize.Unit is %x which is a reserved value\n", __func__, BiosInfo->ExtendedBiosSize.Unit));
    Status = EFI_INVALID_PARAMETER;
    goto exit;
  }
