/** @file
  SMBIOS Type 42 Table Generator.
  ...
  )
{
  ...
  if (RopInfo->Hostname[0] != '\0') {
    RopData->HostnameLength = (UINT8)AsciiStrSize (RopInfo->Hostname);
    AsciiStrCpyS (Hostname, sizeof (RopInfo->Hostname), RopInfo->Hostname);
  }
  ...
  )
{
  ...
  switch (UsbInfo->Version) {
    case 1:
      ByteSize = AsciiStrLen (UsbInfo->SerialNumberStr) * 2;
      if (ByteSize < 2) {
        DEBUG ((
          DEBUG_ERROR,
      ...
      ///
      /// BytesSize is the size of Serial number unicode string without
      /// NULL terminator.
      ///
  ...
  )
{
  ...
  switch (UsbInfo->Version) {
    case 1:
      ...
      UsbData->SerialNumberDescLength = (UINT8)(StrLen (UnicodeStr));
      StrCpyS ((VOID *)(UsbData + 1), UsbData->SerialNumberDescLength + 1, UnicodeStr);
      break;
    case 2:
  ...
  )
{
  ...
  if ((NetDevOps->CmObject != NULL) && (Count != 1)) {
    DEBUG ((
      DEBUG_ERROR,
...
/** Free any resources allocated when installing SMBIOS Type 42 table.
  ...
  )
{
  ...
  *TableCount = 0;
  ...
  for (InterfaceIdx = 0; InterfaceIdx < MchiCount; InterfaceIdx++) {
    ...
    if ((DataOps->CmObject != NULL) && (Count != 1)) {
      DEBUG ((
        DEBUG_ERROR,
