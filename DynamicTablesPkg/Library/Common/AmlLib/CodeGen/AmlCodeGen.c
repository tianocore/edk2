...
/** Create a _CST node.
  ...
  @param [in]  CstInfo      Array of CstInfo object.
  ...
  )
{
  ...
  AML_OBJECT_NODE_HANDLE  CstPackage;
  AML_OBJECT_NODE_HANDLE  IntegerNode;
  AML_OBJECT_NODE_HANDLE  CstSubPackage;
  UINT32                  Index;
  ...
  Status = AmlCodeGenNamePackage ("_CST", NULL, &CstNode);
  if (EFI_ERROR (Status)) {
    ASSERT_EFI_ERROR (Status);
    return Status;
  }
  ...
  // Get the Package object node of the _CST node,
  // which is the 2nd fixed argument (i.e. index 1).
  CstPackage = (AML_OBJECT_NODE_HANDLE)AmlGetFixedArgument (
                                         CstNode,
                                         EAmlParseIndexTerm1
                                         );
  if ((CstPackage == NULL)                                              ||
      (AmlGetNodeType ((AML_NODE_HANDLE)CstPackage) != EAmlNodeObject)  ||
      (!AmlNodeHasOpCode (CstPackage, AML_PACKAGE_OP, 0)))
  {
    Status = EFI_INVALID_PARAMETER;
    goto error_handler;
  }

  Status = AmlCodeGenInteger (NumEntries, &IntegerNode);
  if (EFI_ERROR (Status)) {
    goto error_handler;
  }

  Status = AmlVarListAddTail (
             (AML_NODE_HANDLE)CstPackage,
             (AML_NODE_HANDLE)IntegerNode
             );
  if (EFI_ERROR (Status)) {
    FreePool (IntegerNode);
  ...
  for (Index = 0; Index < NumEntries; Index++) {
    Status = AmlCodeGenPackage (&CstSubPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterToPackage ((VOID *)&CstInfo[Index].Register, CstSubPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }
  ...
  Status = LinkNode (CstNode, ParentNode, NewCstNode);
  if (EFI_ERROR (Status)) {
    goto error_handler;
  }
...
error_handler:
  ASSERT_EFI_ERROR (Status);
  AmlDeleteTree ((AML_NODE_HANDLE)CstNode);
  if (CstSubPackage != NULL) {
    AmlDeleteTree ((AML_NODE_HANDLE)CstSubPackage);
  }
  ...
  )
{
  ...
  for (Index = 0; Index < NumEntries; Index++) {
    if ((CsdInfo[Index].NumEntries != EFI_ACPI_6_5_AML_CSD_NUM_ENTRIES) ||
        (CsdInfo[Index].Revision != EFI_ACPI_6_5_AML_CSD_REVISION)      ||
  ...
  for (Index = 0; Index < NumEntries; Index++) {
    ...
    Status = AmlAddRegisterOrIntegerToPackage (NULL, CsdInfo[Index].NumEntries, CsdPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, CsdInfo[Index].Revision, CsdPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, CsdInfo[Index].Domain, CsdPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, CsdInfo[Index].CoordType, CsdPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, CsdInfo[Index].NumProcessors, CsdPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, CsdInfo[Index].Index, CsdPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }
  ...
  )
{
  ...
  Status = AmlAddRegisterToPackage ((VOID *)&PctInfo->ControlRegister, PctPackage);
  if (EFI_ERROR (Status)) {
    goto error_handler;
  }

  Status = AmlAddRegisterToPackage ((VOID *)&PctInfo->StatusRegister, PctPackage);
  if (EFI_ERROR (Status)) {
    goto error_handler;
  }
  ...
  )
{
  ...
  for (Index = 0; Index < NumPackages; Index++) {
    ...
    Status = AmlAddRegisterOrIntegerToPackage (NULL, PssInfo[Index].CoreFrequency, PssPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, PssInfo[Index].Power, PssPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, PssInfo[Index].Latency, PssPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, PssInfo[Index].BusMasterLatency, PssPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, PssInfo[Index].Control, PssPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }

    Status = AmlAddRegisterOrIntegerToPackage (NULL, PssInfo[Index].Status, PssPackage);
    if (EFI_ERROR (Status)) {
      goto error_handler;
    }
