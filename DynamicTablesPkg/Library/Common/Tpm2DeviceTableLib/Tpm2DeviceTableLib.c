...
/** Fixup the Tpm2 device _CRS values (BaseAddress, ...).
  ...
  @param  [in]  RootNodeHandle  Pointer to the root of an AML tree.
  @param  [in]  TpmInfo         Pointer to a Serial Tpm Information
                                structure.
...
/** Fixup the Tpm2 Device in the AML tree.
  ...
  @param  [in]  TpmInfo         Pointer to a Serial Tpm2 Information
                                structure.
  @param  [in]  Name            The Name to give to the Device.
                                Must be a NULL-terminated ASL NameString
                                e.g.: "DEV0", "DV15.DEV0", etc.
  ...
  )
{
  ...
  Status = FixupTpm2DeviceCrs (RootNodeHandle, TpmInfo);
  if (EFI_ERROR (Status)) {
    return Status;
  }
