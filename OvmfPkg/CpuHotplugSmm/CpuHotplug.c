...
//
// The first array stores APIC IDs for hot-plug events, and the second and the
// third store (and are indexed similarly) APIC IDs and QEMU CPU Selectors for
  ...
  )
{
  ...
  // after having been cleared to exit the SMI and have no SMM
  // processing remaining.
  //
  // Keep them are penned here until the BSP tells QEMU to eject
  // them.
  //
  for (;;) {
    DisableInterrupts ();
