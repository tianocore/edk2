...
/**
 * Measure TdHob passed from VMM and store the measurement value in workarea.
 *
 * @param  TdHob        Pointer to the TdHob
 *
 * @retval EFI_SUCCESS  Successfully meaure the TdHob
  ...
  )
{
  ...
  WorkArea->TdxWorkArea.SecTdxWorkArea.TdHobMeasurement.Signature = SIGNATURE_32 ('T', 'D', 'H', 'B');
  CopyMem (WorkArea->TdxWorkArea.SecTdxWorkArea.TdHobMeasurement.HashValue, Digest, SHA384_DIGEST_SIZE);
  ...
  )
{
  ...
  WorkArea->TdxWorkArea.SecTdxWorkArea.CfvMeasurement.Signature = SIGNATURE_32 ('T', 'C', 'F', 'V');
  CopyMem (WorkArea->TdxWorkArea.SecTdxWorkArea.CfvMeasurement.HashValue, Digest, SHA384_DIGEST_SIZE);
