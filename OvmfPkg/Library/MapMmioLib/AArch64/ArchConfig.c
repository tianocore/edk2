/** @file
  Arch specific configuration interfaces.
...
/**
  ...
  On AArch64, when running as an Arm CCA Realm, this function marks the
  range as shared so the Realm can access host-provided MMIO regions.
  ...
  )
{
  ...
  Status = ArmCcaSetMemoryProtectionAttribute (
             RegionBase,
             RegionSize,
             TRUE
             );
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "Failed to set Arm CCA memory Protection Attribute. Status = %r\n",
      Status
      ));
  }
