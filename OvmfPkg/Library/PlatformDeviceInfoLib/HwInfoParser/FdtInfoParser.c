...
{
  PLATFORM_DEVICE_INFO_CONTEXT  *PlatInfoCtx;
  UINTN                         Index;
  ...
  PlatInfoCtx = (PLATFORM_DEVICE_INFO_CONTEXT *)Context;
  if (PlatInfoCtx->DevCount >= PlatInfoCtx->MaxDevices) {
    DEBUG ((DEBUG_ERROR, "Error: Maximum platform device count exceeded!\n"));
    ASSERT (FALSE);
    return EFI_OUT_OF_RESOURCES;
  }

  Index                               = PlatInfoCtx->DevCount++;
  PlatInfoCtx->Dev[Index].BaseAddress = Base;
  PlatInfoCtx->Dev[Index].Length      = Range;
  AsciiStrnCpyS (PlatInfoCtx->Dev[Index].Desc, 16, Desc, AsciiStrLen (Desc));
  ...
  )
{
  ...
  if ((FdtBase == NULL) ||
      ...
      (MaxDevices == NULL) ||
      (*MaxDevices == 0) ||
      (FdtCheckHeader (FdtBase) != 0))
  {
    return EFI_INVALID_PARAMETER;
  }

  PlatInfoCtx.Dev        = PlatDevInfo;
  PlatInfoCtx.MaxDevices = *MaxDevices;
  PlatInfoCtx.DevCount   = 0;

  Parser.Fdt           = FdtBase;
  Parser.HwAddressInfo = HwAddressInfo;
  Parser.Context       = &PlatInfoCtx;

  Status = MainDispatcher (&Parser, 0);
  if (!EFI_ERROR (Status)) {
    *MaxDevices = PlatInfoCtx.DevCount;
  } else {
    *MaxDevices = 0;
  }
