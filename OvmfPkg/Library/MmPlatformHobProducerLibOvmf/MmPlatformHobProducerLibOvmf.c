  ...
  )
{
  ...
  OVMF_MEMORY_ENCRYPTION_CONFIG  *MemEncryptConfigHob;
  ...
  // Account for the Memory Encryption HOB
  Size += (sizeof (EFI_HOB_GUID_TYPE) + sizeof (OVMF_MEMORY_ENCRYPTION_CONFIG));
  ...
  *BufferSize = Size;
  ...
  // Create the Memory Encryption HOB
  GuidHob                   = (EFI_HOB_GUID_TYPE *)((UINT8 *)Buffer + Size);
  GuidHob->Header.HobType   = EFI_HOB_TYPE_GUID_EXTENSION;
  GuidHob->Header.HobLength = (UINT16)(sizeof (EFI_HOB_GUID_TYPE) + sizeof (OVMF_MEMORY_ENCRYPTION_CONFIG));
  GuidHob->Header.Reserved  = 0;
  CopyMem (&GuidHob->Name, &gOvmfMemoryEncryptionConfigGuid, sizeof (EFI_GUID));
  MemEncryptConfigHob                                   = (OVMF_MEMORY_ENCRYPTION_CONFIG *)((UINT8 *)GuidHob + sizeof (EFI_HOB_GUID_TYPE));
  MemEncryptConfigHob->ConfidentialComputingGuestAttr   = PcdGet64 (PcdConfidentialComputingGuestAttr);
  MemEncryptConfigHob->PteMemoryEncryptionAddressOrMask = PcdGet64 (PcdPteMemoryEncryptionAddressOrMask);
  Size                                                 += GuidHob->Header.HobLength;
  Size                                                  = ALIGN_VALUE (Size, 8);
