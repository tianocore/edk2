...
/**
   ...
   This function decides which paging mode will be used by OVMF going forward.

   When running in 4-level paging mode (due to the ResetVector entering long
   mode that way) nothing will be changed.  The function will never try to
   upgrade to 5-level paging.

   When running in 5-level paging mode this function looks at the virtual
   machine configuration, then decides whenever it will continue to use 5-level
   paging or downgrade to 4-level paging for better compatibility with older
   guest OS versions:
     ...
     option is present the function will use the requested paging mode.
