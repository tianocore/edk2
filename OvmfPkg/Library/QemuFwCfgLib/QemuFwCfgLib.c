  ...
  )
{
  ...
  if ((FwCfgCacheWorkArea->Offset + Size) > MAX_UINT32) {
    DEBUG ((DEBUG_ERROR, "%a: Item Offset Integer Overflow ", __func__));
    ASSERT (FALSE);
    return;
  }

  FwCfgCacheWorkArea->Offset += Size;
}
  ...
  )
{
  ...
  RStatus = InternalQemuFwCfgItemInCacheList (Name, Item, Size);
  if (RStatus != RETURN_UNSUPPORTED) {
    return RStatus;
  }
