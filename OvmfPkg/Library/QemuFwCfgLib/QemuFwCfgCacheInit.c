  ...
  )
{
  ...
  FileDirSize = FileCount * sizeof (FWCFG_FILE) + sizeof (FW_CFG_FILE_DIR_HOB_HEADER);
  DEBUG ((DEBUG_INFO, "%a: FileDir TotalSize is %d (Bytes)\n", __func__, FileDirSize));
  FwCfginfoHobData = BuildGuidHob (&gOvmfFwCfgInfoHobGuid, FileDirSize);
  if (FwCfginfoHobData == NULL) {
    DEBUG ((DEBUG_ERROR, "%a: BuildGuidHob Failed with FileDirSize(0x%x)\n", __func__, FileDirSize));
    return EFI_OUT_OF_RESOURCES;
  }

  ZeroMem (FwCfginfoHobData, FileDirSize);
  FW_CFG_FILE_DIR_HOB_HEADER  *FileDirHobHeader;
  UINT8                       *Ptr;

  FileDirHobHeader            = (FW_CFG_FILE_DIR_HOB_HEADER *)FwCfginfoHobData;
  FileDirHobHeader->FwCfgItem = QemuFwCfgItemFileDir;
  FileDirHobHeader->ItemCount = FileCount;

  Ptr = FwCfginfoHobData + sizeof (FW_CFG_FILE_DIR_HOB_HEADER);
  QemuFwCfgReadBytes (FileDirSize, Ptr);
  ...
  )
{
  ...
  if (EFI_ERROR (CacheQemuFwCfgFileDirData ())) {
    return EFI_UNSUPPORTED;
  }
