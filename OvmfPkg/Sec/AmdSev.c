...
{
  MSR_SEV_ES_GHCB_REGISTER  Msr;
  EFI_PHYSICAL_ADDRESS      ReqGuestFrameNumber;
  EFI_PHYSICAL_ADDRESS      RespGuestFrameNumber;

  ReqGuestFrameNumber = RShiftU64 ((UINT64) Address, EFI_PAGE_SHIFT);

  //
  // Use the GHCB MSR Protocol to request to register the GPA.
  //
  Msr.GhcbPhysicalAddress = LShiftU64 (ReqGuestFrameNumber, 12);
  Msr.GhcbGpaRegister.Function = GHCB_INFO_GHCB_GPA_REGISTER_REQUEST;
  //Msr.GhcbGpaRegister.GuestFrameNumber = ReqGuestFrameNumber;
  AsmWriteMsr64 (MSR_SEV_ES_GHCB, Msr.GhcbPhysicalAddress);
  ...
  //
  RespGuestFrameNumber = RShiftU64 (Msr.GhcbPhysicalAddress, 12);
  if ((Msr.GhcbGpaRegister.Function != GHCB_INFO_GHCB_GPA_REGISTER_RESPONSE) ||
      (RespGuestFrameNumber != ReqGuestFrameNumber)) {
