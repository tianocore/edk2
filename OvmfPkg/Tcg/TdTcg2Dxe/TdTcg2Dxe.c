  ...
  )
{
  ...
  if (PcdGet8 (PcdTpmPlatformClass) == TCG_PLATFORM_TYPE_SERVER) {
    ...
    if (!EFI_ERROR (Status)) {
      CcEvent.MrIndex   = TdxMapPcrToMrIndex (1);
  ...
  )
{
  ...
  CcEvent.MrIndex   = TdxMapPcrToMrIndex (7);
  CcEvent.EventType = EV_EFI_ACTION;
  ...
  )
{
  ...
  //
  Status = MeasureSeparatorEvent (TdxMapPcrToMrIndex (7));
