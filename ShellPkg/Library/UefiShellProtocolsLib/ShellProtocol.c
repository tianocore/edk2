  ...
  )
{
  ...
  *FileHandle  = NULL;
    ...
    if (Handle1 != NULL) {
      ...
      if (!EFI_ERROR (Status)) {
              ...
              )
        {
          ...
          // Close the last node
          //
          ShellProtocolsInfoObject.NewEfiShellProtocol->CloseFile (Handle2);
  ...
  if (EFI_ERROR (Status)) {
    if (Handle1 != NULL) {
      ShellProtocolsInfoObject.NewEfiShellProtocol->CloseFile (Handle1);
    }
  } else {
  ...
  )
{
  ...
  *FileHandle = NULL;
  ...
  if (StrCmp (FileName, L">i") == 0) {
    ...
    *FileHandle = ShellProtocolsInfoObject.NewShellParametersProtocol->StdIn;
  ...
  )
{
  ...
  //
  ShellFileHandleRemove (FileHandle);
  return (ShellProtocolsInfoObject.NewEfiShellProtocol->DeleteFile (FileHandle));
}
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (LoadedImage->ImageCodeType != EfiLoaderCode) {
      ShellPrintHiiEx (
        ...
        ShellProtocolsInfoObject.HiiHandle
        );
      goto UnloadImage;
    }
    ...
    //
    ShellParamsProtocol.StdIn  = ShellProtocolsInfoObject.NewShellParametersProtocol->StdIn;
    ShellParamsProtocol.StdOut = ShellProtocolsInfoObject.NewShellParametersProtocol->StdOut;
    ShellParamsProtocol.StdErr = ShellProtocolsInfoObject.NewShellParametersProtocol->StdErr;
  ...
  )
{
  ...
  if (NestingEnabled ()) {
    DevPath = AppendDevicePath (ShellProtocolInteractivityInfoObject.ImageDevPath, ShellProtocolInteractivityInfoObject.FileDevPath);

    DEBUG_CODE_BEGIN ();
    Temp = ConvertDevicePathToText (ShellProtocolInteractivityInfoObject.FileDevPath, TRUE, TRUE);
    FreePool (Temp);
    Temp = ConvertDevicePathToText (ShellProtocolInteractivityInfoObject.ImageDevPath, TRUE, TRUE);
    FreePool (Temp);
  ...
  )
{
  ...
  if (FileListNode->Handle != NULL) {
    ShellProtocolsInfoObject.NewEfiShellProtocol->CloseFile (FileListNode->Handle);
  }
  ...
  )
{
  ...
  if ((StrCmp (Name, L"cwd") == 0) ||
      ...
      (!ShellProtocolsInfoObject.ShellProtocolBitUnion.Bits.NoNest &&
...
{
  return (ShellProtocolsInfoObject.RootShellInstance);
}
  ...
  )
{
  ...
  InitializeListHead (&ShellProtocolsInfoObject.OldShellList.Link);
  ...
  if (Status == EFI_BUFFER_TOO_SMALL) {
    ...
    for (HandleCounter = 0; HandleCounter < (BufferSize/sizeof (EFI_HANDLE)); HandleCounter++) {
      ...
      if (!EFI_ERROR (Status)) {
        ...
        if (OldProtocolNode == NULL) {
          if (!IsListEmpty (&ShellProtocolsInfoObject.OldShellList.Link)) {
        ...
        if (!EFI_ERROR (Status)) {
          ...
          //
          InsertTailList (&ShellProtocolsInfoObject.OldShellList.Link, &OldProtocolNode->Link);
        }
      }
    }
    ...
    ASSERT (IsListEmpty (&ShellProtocolsInfoObject.OldShellList.Link));
    //
  ...
  )
{
  ...
  //
  if (!IsListEmpty (&ShellProtocolsInfoObject.OldShellList.Link)) {
    for (Node2 = (SHELL_PROTOCOL_HANDLE_LIST *)GetFirstNode (&ShellProtocolsInfoObject.OldShellList.Link)
         ; !IsListEmpty (&ShellProtocolsInfoObject.OldShellList.Link)
         ; Node2 = (SHELL_PROTOCOL_HANDLE_LIST *)GetFirstNode (&ShellProtocolsInfoObject.OldShellList.Link)
         )
    {
  ...
  )
{
  ...
  Status = CleanUpInteractiveShellEnvironment ();
  return Status;
}
