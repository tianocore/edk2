  ...
  )
{
  ...
  Temp = NULL;
  if (NestingEnabled ()) {
    DevPath = AppendDevicePath (ShellProtocolsInfoObject.ImageDevPath, ShellProtocolsInfoObject.FileDevPath);
    if (DevPath == NULL) {
      return EFI_OUT_OF_RESOURCES;
    }

    DEBUG_CODE_BEGIN ();
    Temp = ConvertDevicePathToText (ShellProtocolsInfoObject.FileDevPath, TRUE, TRUE);
    if (Temp != NULL) {
      FreePool (Temp);
    }

    Temp = ConvertDevicePathToText (ShellProtocolsInfoObject.ImageDevPath, TRUE, TRUE);
    if (Temp != NULL) {
      FreePool (Temp);
    }
