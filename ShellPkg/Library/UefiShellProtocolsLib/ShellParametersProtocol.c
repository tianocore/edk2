  ...
  )
{
  ...
  *OldStdErr                    = ShellParameters->StdErr;
  ...
  if (Status == EFI_NOT_FOUND) {
    ShellPrintHiiEx (-1, -1, NULL, STRING_TOKEN (STR_SHELL_REDUNDA_REDIR), ShellProtocolsInfoObject.HiiHandle);
  } else if (EFI_ERROR (Status)) {
    ShellPrintHiiEx (-1, -1, NULL, STRING_TOKEN (STR_SHELL_INVALID_REDIR), ShellProtocolsInfoObject.HiiHandle);
  }
