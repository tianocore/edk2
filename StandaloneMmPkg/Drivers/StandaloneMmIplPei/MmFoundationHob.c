  ...
  )
{
  ...
  DataSize = sizeof (EFI_MMRAM_HOB_DESCRIPTOR_BLOCK) + (MmramBlocks->NumberOfMmReservedRegions - 1) * sizeof (EFI_MMRAM_DESCRIPTOR);
  ...
  )
{
  ...
  if (GuidHob == NULL) {
    DEBUG ((DEBUG_WARN, "GUIDed HOB %g could not be found", Guid));
  }
