  ...
  )
{
  ...
  if ((EventDataSize == sizeof (TCG_EfiStartupLocalityEvent)) &&
      (CompareMem (&StartupLocalityEvent->Signature, TCG_EfiStartupLocalityEvent_SIGNATURE, 16) == 0))
  {
    /*
...
/**
  Get Tpm event log from transfer list.

  @param[in]   TransferList           Transfer list
  @param[out]  EventLog               TPM event log
  @param[out]  EventLogSize           Total size of TPM event log
...
{
  TRANSFER_ENTRY_HEADER  *TransferEntry;

  TransferEntry = TransferListFindFirstEntry (TransferList, TRANSFER_ENTRY_TAG_ID_TPM_EVENT_LOG);
  if (TransferEntry == NULL) {
    DEBUG ((DEBUG_ERROR, "%a: No TPM event log entry found...\n", __func__));
    return EFI_NOT_FOUND;
  }

  /*
   * The first 4 bytes are flags.
   * See TPM event log table entry layout (XFERLIST_EVLOG) in
   * https://github.com/FirmwareHandoff/firmware_handoff/blob/main/source/transfer_list.rst#id25
   */
  *EventLog = TransferListGetEntryData (TransferEntry) + sizeof (UINT32);
  if ((*EventLog == NULL) || (TransferEntry->DataSize - sizeof (UINT32) == 0)) {
    DEBUG ((DEBUG_WARN, "%a: No data for Tpm event log...\n", __func__));
    return EFI_NOT_FOUND;
  }

  *EventLogSize = TransferEntry->DataSize - sizeof (UINT32);

  if (*EventLogSize < OFFSET_OF (TCG_PCR_EVENT, Event)) {
    DEBUG ((DEBUG_WARN, "%a: Invalid event log size...\n", __func__));
    return EFI_NOT_FOUND;
  }
  ...
  )
{
  ...
  Status = GetTpmEventLog (TransferList, &EventLog, &EventLogSize);
  if (EFI_ERROR (Status)) {
    /*
  ...
  if (!IsSpecIdEvent (NextEvent)) {
    /*
     * First event should be SpecId event otherwise it's invalid event log.
     * skip creating EventlogHobs from TransferList.
     */
    DEBUG ((DEBUG_WARN, "%a: Invalid TPM event log...", __func__));
    return EFI_SUCCESS;
  }
  ...
  /*
   * By Tcg2Dxe, SpecId Event is created when it generates final event log.
   * So, Skip creation PeiEventLogHob for SpecIdEvent
   */
