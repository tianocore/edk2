  ...
  )
{
  ...
  for (Index = 0; Index < MicrocodePatchHob->ProcessorCount; Index++) {
    if (MAX_UINT64 == MicrocodePatchHob->ProcessorSpecificPatchOffset[Index]) {
      ...
      MicrocodePatchesList[Index].Address     = MicrocodePatchHob->MicrocodePatchAddress + (UINTN)(MicrocodePatchHob->ProcessorSpecificPatchOffset[Index]);
      MicrocodePatchesList[Index].Size        = ((CPU_MICROCODE_HEADER*)(MicrocodePatchHob->MicrocodePatchAddress + (UINTN)(MicrocodePatchHob->ProcessorSpecificPatchOffset[Index])))->TotalSize;
