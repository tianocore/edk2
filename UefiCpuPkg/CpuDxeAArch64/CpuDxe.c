  ...
  )
{
  ...
  UINTN  Idx                  = 0;
  UINTN  GicInterfaceBase     = (UINTN)PcdGet64 (PcdGicInterruptInterfaceBase);
  ...
  while (Hob.Raw != NULL) {
    ...
    if (Resource->PhysicalStart == GicInterfaceBase) {
      SkipGici = 1;
    }
  ...
  if (SkipGici == 0) {
    VirtualMemoryTable[Idx].PhysicalBase = GicInterfaceBase;
    VirtualMemoryTable[Idx].VirtualBase  = VirtualMemoryTable[Idx].PhysicalBase;
