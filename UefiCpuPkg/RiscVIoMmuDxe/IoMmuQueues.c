...
/**
  A helper function that detects if a queue is full, handling wraparound.

**/
STATIC
BOOLEAN
IoMmuQueueIsFull (
  IN UINT32  QueueHeadIndex,
  IN UINT32  QueueTailIndex
  )
{
  if ((QueueHeadIndex == 0) && (QueueTailIndex == (QUEUE_NUMBER_OF_ENTRIES - 1))) {
    return TRUE;
  }

  return QueueTailIndex == (QueueHeadIndex - 1);
}

/**
  IOMMU command-queue worker.

  @param[in]  IoMmuContext   The context struct of an IOMMU.
  @param[in]  CommandBuffer  A command-sized buffer to enqueue.

  @retval EFI_SUCCESS        The command was added to the queue.
  @retval EFI_DEVICE_ERROR   The command queue is full.
  ...
  )
{
  ...
  //
  // At the minimum it can be (a page), the queue is quite long when considering its fencing.
  // Therefore, just ASSERT and return.
  //
  ...
  if (IoMmuQueueIsFull (QueueHead.Bits.index, QueueTail.Bits.index)) {
    ASSERT (FALSE);
...
/**
  ...
  FIXME: Determine if this must be called in a loop for *multiple* pages.
  ...
  )
{
  ...
  //
  // If the needed width is sufficiently high, then the segment number must be passed too.
  //
  ...
  )
{
  ...
  //
  // As a likely performance optimisation, only command invalidation of its entries.
  //
  Command.DV  = 1;
  ...
  )
{
  ...
  // Therefore, if the IOMMU reports one, print it, handle it, and ASSERT.
  //
  CqCsr.Uint32 = MmioRead32 (IoMmuContext->BaseAddress + R_RISCV_IOMMU_CQCSR);
