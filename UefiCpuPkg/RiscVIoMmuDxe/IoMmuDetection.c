  ...
  )
{
  ...
  //
  // Try to locate the protocol because registering for the notification triggers the event too.
  //
  ...
  //
  // Iterate all PCI devices, looking for those that are IOMMUs and were discovered previously
  // to initialise now that resources are known.
  //
  // Once all IOMMUs are initialised, this loop will waste some time iterating over the remaining PCI devices.
  // However, it's not worth making the context scanning code more complex by scanning for this too,
  // as there aren't many PCI devices or IOMMU contexts.
  //
  for (Index = 0; Index < HandleCount; Index++) {
    ...
    // Failing to match any context means that this device isn't an IOMMU.
    ...
    Data   = EFI_PCI_COMMAND_MEMORY_SPACE;
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_WARN,
        "The RISC-V IOMMU PCI device at 0x%lx failed to initialise\n",
  ...
  )
{
  ...
  while (IoMmuNode != -FDT_ERR_NOTFOUND) {
    ...
    if (FdtStringListContains (PropertyPtr, TempLen, "riscv,iommu")) {
      ...
      StartAddress = Fdt32ToCpu (Data32);
    ...
    IoMmuContext->TempHandle = Fdt32ToCpu (Data32);
...
NextIoMmuNode:
  ...
  while (DownstreamDeviceNode != -FDT_ERR_NOTFOUND) {
    ...
    // Even with information in the FDT/ACPI, this is unimplementable by common code for platform devices,
    // because we can't construct a device_id from the DevicePath.
    //
    ...
    if (Data32 != NULL) {
      IoMmuMapMask = Fdt32ToCpu (Data32);
    }
    ...
    for (Index = 0; Index < TempLen / sizeof (UINT32); Index += 4) {
      MapLength = Fdt32ToCpu (Data32 + Index + 3);
      if (MapLength == 0) {
        continue;
      }

      MapPhandle = Fdt32ToCpu (Data32 + Index + 1);
      for (Link = GetFirstNode (&mRiscVIoMmuContexts)
           ; !IsNull (&mRiscVIoMmuContexts, Link)
           ; Link = GetNextNode (&mRiscVIoMmuContexts, Link)
           )
      ...
      IoMmuDownstreams->NodeMapping.SourceIdBase            = Fdt32ToCpu (Data32 + Index);
      IoMmuDownstreams->NodeMapping.NumberOfIDs             = MapLength;
      IoMmuDownstreams->NodeMapping.DestinationDeviceIdBase = Fdt32ToCpu (Data32 + Index + 2);
  ...
  )
{
  ...
  AcpiRimtTable = (VOID *)EfiLocateFirstAcpiTable (EFI_ACPI_6_6_RIMT_TABLE_SIGNATURE);
  ...
  for (NodeIndex = 0; NodeIndex < AcpiRimtTable->NumberOfRimtNodes; NodeIndex++) {
    ...
    if ((RimtPcieNode->PcieSegmentNumber + 1) > mPciSegmentCount) {
      mPciSegmentCount = RimtPcieNode->PcieSegmentNumber + 1;
    }
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    return IoMmuDeviceTreeDiscovery ();
  }
