...
/**
  Determines the top of IOMMU addressable memory based on
  the HART's operating SATP mode and the IOMMU's GXL bit.

  @return  The top of IOMMU addressable memory.
  ...
  )
{
  ...
  //
  // If this is a dedicated buffer, check that it can fit on a page.
  //
