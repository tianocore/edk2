...
//
// The number of segments informs the needed device_id width.
//
UINT8  mPciSegmentCount = 0;
  ...
  )
{
  ...
  // such as searching the FQ. The command queue is used synchronously.
  //
  ...
  )
{
  ...
  NeededDeviceIdWidth = IoMmuNeededDeviceIdWidth ();
  ...
  if (NeededDeviceIdWidth <= OneLevelMaxWidth) {
    IoMmuMode = V_RISCV_IOMMU_DDTP_IOMMU_MODE_1LVL;
  } else if (NeededDeviceIdWidth <= TwoLevelMaxWidth) {
    IoMmuMode = V_RISCV_IOMMU_DDTP_IOMMU_MODE_2LVL;
  } else if (NeededDeviceIdWidth <= 24) {
  ...
  )
{
  ...
  //
  return InitialiseRiscVIoMmu (IoMmuContext);
}
  ...
  )
{
  ...
  while (!IsNull (&mRiscVIoMmuContexts, Link))
  {
    IoMmuContext = RISCV_IOMMU_CONTEXT_FROM_LINK (Link);

    DownstreamLink = GetFirstNode (&IoMmuContext->DownstreamDevices);
    while (!IsNull (&IoMmuContext->DownstreamDevices, DownstreamLink))
    {
  ...
  )
{
  ...
  Status = DetectRiscVIoMmus ();
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "No RISC-V IOMMUs could be found\n"));
    return EFI_SUCCESS;
  }
       ...
       )
  {
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_WARN,
        "The RISC-V IOMMU %a device at 0x%lx failed to initialise\n",
        IoMmuContext->IoMmuIsPciDevice ? "PCI" : "system",
        IoMmuContext->BaseAddress
        ));
    }
