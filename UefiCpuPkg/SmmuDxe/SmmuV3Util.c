  ...
  do {
    ...
    if (IsEmpty == FALSE) {
      ...
      if (IsConfigErr) {
        ...
        DEBUG ((
          DEBUG_ERROR,
          "%a: SMMU 0x%llx IDR0=0x%08x IDR1=0x%08x IDR3=0x%08x IDR5=0x%08x TranslationStage=%u Flags=0x%x\n",
          __func__,
          SmmuInfo->SmmuBase,
          Idr0Raw,
          Idr1Raw,
        ...
        if (SteSlot != NULL) {
          ...
          if ((SmmuInfo->TranslationStage == SmmuTranslationStage1) && (SteSlot->Bits.S1ContextPtr != 0)) {
      ...
      // Dump PTE's if translation related fault. Stage 2: STE.S2Ttb.
      // Stage 1: STE.S1ContextPtr -> CD.Ttb0.
      if (IsTranslErr) {
        ...
        // the stage-appropriate root (Stage 2: S2Ttb; Stage 1: CD.Ttb0)
        ...
        if (SteSlot != NULL) {
          if (SmmuInfo->TranslationStage == SmmuTranslationStage1) {
            ...
            DEBUG ((
              DEBUG_ERROR,
              "%a: Active Stage 2 STE for StreamId=0x%x: Valid=%u Config=0x%x S2Ttb=0x%llx S2Vmid=0x%x Root=%p\n",
              __func__,
              FaultStreamId,
              SteSlot->Bits.Valid,
              SteSlot->Bits.Config,
              (UINT64)SteSlot->Bits.S2Ttb,
              (UINT32)SteSlot->Bits.S2Vmid,
              FaultRoot
              ));
          }
        } else {
        ...
        if ((SteSlot != NULL) && (SteSlot->Bits.Valid != 0) && (FaultRoot != NULL)) {
          ...
          // STE with S1ContextPtr = 0, or Stage 2 STE with S2Ttb = 0).
  ...
  )
{
  ...
  // invalidations (CMD_TLBI_*) are completed
