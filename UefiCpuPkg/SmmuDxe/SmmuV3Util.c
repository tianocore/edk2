  ...
  )
{
  ...
  //
  // A single-page L1 root covers up to PAGE_TABLE_L1_ADDRESS_WIDTH_MAX
  // bits; L0 covers the full 48-bit input at 4KB granule.
  //
  if (OutputAddressWidth > PAGE_TABLE_L1_ADDRESS_WIDTH_MAX) {
  ...
  )
{
  ...
  for (PageIndex = 0; PageIndex < Pages; PageIndex++) {
    ...
    for (Level = SmmuInfo->TranslationStartingLevel; Level < PAGE_TABLE_DEPTH - 1; Level++) {
      Index = PAGE_TABLE_INDEX (PageAddress, Level);
      Entry = Current->Entries[Index];
  ...
  )
{
  ...
  do {
    ...
    if (IsEmpty == FALSE) {
      ...
      FaultType   = (UINT8)(FaultRecord.Fault[0] & 0xFF);
      IsTranslErr = (FaultType == 0x10) || (FaultType == 0x11) || (FaultType == 0x12) || (FaultType == 0x13);
      // Config-error class: C_BAD_STREAMID / C_BAD_STE / C_BAD_SUBSTREAMID / C_BAD_CD.
      IsConfigErr = (FaultType == 0x02) || (FaultType == 0x04) || (FaultType == 0x08) || (FaultType == 0x0A);

      // On config-error faults dump raw STE bytes and IDR fingerprint so the offending field is visible.
      if (IsConfigErr) {
        UINT32  Idr0Raw;
        UINT32  Idr1Raw;
        UINT32  Idr3Raw;
        ...
        Idr0Raw = SmmuV3ReadRegister32 (SmmuInfo->SmmuBase, SMMU_IDR0);
        Idr1Raw = SmmuV3ReadRegister32 (SmmuInfo->SmmuBase, SMMU_IDR1);
        Idr3Raw = SmmuV3ReadRegister32 (SmmuInfo->SmmuBase, SMMU_IDR3);
        Idr5Raw = SmmuV3ReadRegister32 (SmmuInfo->SmmuBase, SMMU_IDR5);
        DEBUG ((
          DEBUG_ERROR,
          "%a: SMMU 0x%llx IDR0=0x%08x IDR1=0x%08x IDR3=0x%08x IDR5=0x%08x Flags=0x%x\n",
          __func__,
          SmmuInfo->SmmuBase,
          Idr0Raw,
          Idr1Raw,
        ...
        OldTpl  = gBS->RaiseTPL (TPL_HIGH_LEVEL);
        SteSlot = SmmuV3GetSteSlot (SmmuInfo, FaultRecord.Translation.StreamId);
        if (SteSlot != NULL) {
          DEBUG ((
            DEBUG_ERROR,
            "%a: STE bytes for StreamId=0x%x SteSlot=%p:\n"
            "  [0]=0x%016llx [1]=0x%016llx [2]=0x%016llx [3]=0x%016llx\n"
            "  [4]=0x%016llx [5]=0x%016llx [6]=0x%016llx [7]=0x%016llx\n",
            __func__,
            FaultRecord.Translation.StreamId,
            SteSlot,
            SteSlot->AsUINT64[0],
            SteSlot->AsUINT64[1],
            SteSlot->AsUINT64[2],
          ...
          if (SteSlot->Bits.S1ContextPtr != 0) {
            SMMUV3_CONTEXT_DESCRIPTOR  *DumpCd;

            DumpCd = (SMMUV3_CONTEXT_DESCRIPTOR *)(UINTN)((UINT64)SteSlot->Bits.S1ContextPtr << SMMUV3_STREAM_TABLE_ENTRY_S1CONTEXTPTR_OFFSET);
            DEBUG ((
              DEBUG_ERROR,
              "%a: CD bytes at %p:\n"
              "  [0]=0x%016llx [1]=0x%016llx [2]=0x%016llx [3]=0x%016llx\n"
          ...
          DEBUG ((DEBUG_ERROR, "%a: Active STE for StreamId=0x%x: not found\n", __func__, FaultRecord.Translation.StreamId));
        }

        gBS->RestoreTPL (OldTpl);
      }

      // Dump PTE's if translation related fault: STE.S1ContextPtr -> CD.Ttb0.
      if (IsTranslErr) {
        ...
        // CD.Ttb0 as the root to walk the page table.
        SteSlot = SmmuV3GetSteSlot (SmmuInfo, FaultStreamId);
        if (SteSlot != NULL) {
          FaultCd = (SMMUV3_CONTEXT_DESCRIPTOR *)(UINTN)((UINT64)SteSlot->Bits.S1ContextPtr << SMMUV3_STREAM_TABLE_ENTRY_S1CONTEXTPTR_OFFSET);
          if (FaultCd != NULL) {
            FaultRoot = (PAGE_TABLE *)(UINTN)((UINT64)FaultCd->Bits.Ttb0 << SMMUV3_CD_TTB0_OFFSET);
          }

          DEBUG ((
            DEBUG_ERROR,
            "%a: Active Stage 1 STE for StreamId=0x%x: Valid=%u Config=0x%x S1ContextPtr=0x%llx CD=%p ASID=0x%x Ttb0=0x%llx Root=%p\n",
            __func__,
            FaultStreamId,
            SteSlot->Bits.Valid,
            SteSlot->Bits.Config,
            (UINT64)SteSlot->Bits.S1ContextPtr,
            FaultCd,
            (FaultCd != NULL) ? (UINT32)FaultCd->Bits.Asid : 0,
            (FaultCd != NULL) ? (UINT64)FaultCd->Bits.Ttb0 : 0,
            FaultRoot
            ));
        } else {
        ...
        } else if ((SteSlot != NULL) && (SteSlot->Bits.Valid == 0)) {
          DEBUG ((DEBUG_ERROR, "%a: StreamId=0x%x has no page-table root (STE still in invalid mode)\n", __func__, FaultStreamId));
        } else if (SteSlot != NULL) {
          // Valid = 1 but per-stream root could not be decoded (e.g., Stage 1
          // STE with S1ContextPtr = 0).
          DEBUG ((DEBUG_ERROR, "%a: StreamId=0x%x STE Valid=1 but per-stream root could not be decoded\n", __func__, FaultStreamId));
        }
...
/**
  ...
  Per SMMUv3 §5.2, Stage 1 TLB entries are tagged with VMID = 0 when
...
/**
  ...
  we ensure a per-stream Stage 1 page-table root exists (allocating it and
  ...
  while (!IsNull (&SmmuInfo->RmrNodeList, Entry)) {
    ...
    for (IdMapIdx = 0; IdMapIdx < Item->RmrNode->Node.NumIdMappings; IdMapIdx++) {
      for (IdInRange = 0; IdInRange < IdMapping[IdMapIdx].NumIds; IdInRange++) {
        ...
        Root   = NULL;
        TagId  = 0;
        Status = SmmuV3StreamGetOrCreate (SmmuInfo, StreamId, &Root, &TagId);
        if (EFI_ERROR (Status)) {
          DEBUG ((
            DEBUG_ERROR,
  ...
  for (NodeIdx = 0; NodeIdx < Iort->NumNodes; NodeIdx++) {
    if (Node->Type == EFI_ACPI_IORT_TYPE_NAMED_COMP) {
          ...
          (AsciiStrLen (WantedName) < NameMax))
      {
        ...
        // must share the primary's Stage 1 page table, CD, and ASID.
        //
        TotalCount      = 0;
        PrimaryStreamId = 0;
