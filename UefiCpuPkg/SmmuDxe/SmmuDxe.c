...
// IORT byte length paired with mIortData, used by the deferred IORT install path.
UINT32  mIortSize = 0;
...
/**
  Protocol-notify callback fired when a platform installs
  gEfiAcpiTableProtocolGuid after SmmuDxe already checked for it at entry.
  Uses mIortData / mIortSize saved during driver init.
  On success, CloseEvent unregisters the notify so it never fires again
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: ACPI Table Protocol notify fired but LocateProtocol failed: %r\n", __func__, Status));
    ASSERT_EFI_ERROR (Status);
    gBS->CloseEvent (Event);
    return;
  }

  Status = AddIortTable (AcpiTable, mIortData, mIortSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Deferred IORT install failed: %r\n", __func__, Status));
    ASSERT_EFI_ERROR (Status);
  } else {
    DEBUG ((DEBUG_INFO, "%a: IORT installed via ACPI Table Protocol notify\n", __func__));
  }
...
/**
  Install the IORT table via the ACPI Table Protocol if available;
  otherwise arm a protocol-notify to install it if/when the protocol arrives.

  Locates gEfiAcpiTableProtocolGuid internally. If the protocol is not yet
  produced (non-ACPI platform or produced later),
  SmmuV3AcpiTableProtocolNotify is registered as the notify callback and
  consumes mIortData / mIortSize if/when the protocol is produced.

  @param [in]  IortData   Pointer to the IORT table data.
  @param [in]  IortSize   Size of the IORT table data in bytes.

  @retval EFI_SUCCESS  IORT installed or notify armed.
  @retval Other        Install / event / notify-registration failure.
**/
STATIC
EFI_STATUS
SmmuV3InstallOrDeferIortTable (
  IN VOID    *IortData,
  IN UINT32  IortSize
  )
{
  ...
  VOID                     *AcpiNotifyRegistration;

  // ACPI Table Protocol is optional. Non-ACPI (e.g. Device-Tree-only)
  // platforms may not publish it; SMMU HW configuration still proceeds.
  // If a producer arrives after us, the notify below installs IORT then.
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to create ACPI notify event (%r); IORT will not be installed\n", __func__, Status));
    ASSERT_EFI_ERROR (Status);
    return Status;
  }
  ...
  Status = gBS->RegisterProtocolNotify (
                  &gEfiAcpiTableProtocolGuid,
                  AcpiNotifyEvent,
                  &AcpiNotifyRegistration
                  );
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to arm ACPI protocol notify (%r); IORT will not be installed\n", __func__, Status));
    gBS->CloseEvent (AcpiNotifyEvent);
    ASSERT_EFI_ERROR (Status);
    return Status;
  }

  DEBUG ((DEBUG_INFO, "%a: Deferred IORT install: waiting on ACPI Table Protocol\n", __func__));
  return EFI_SUCCESS;
}
...
/**
  ...
  @param [in]  SmmuInfo  SMMU instance the page table belongs to.
  ...
  )
{
  ...
  NumPages = PAGE_TABLE_ROOT_PAGES;

  // Align the single-page root translation table to a 4 KB boundary.
  ...
  )
{
  ...
  // The root is a single page; deeper levels are also single pages
  // allocated by UpdateMapping().
  if (Level == 0) {
    FreePages (PageTable, PAGE_TABLE_ROOT_PAGES);
  } else {
...
/**
  ...
  Implemented by reusing SmmuV3BuildStage1TranslateStreamTableEntry
  with Cd == NULL and VALID = 0.
  ...
  @retval Other                  Failure from the Stage 1 STE builder.
**/
EFI_STATUS
  ...
  )
{
  ...
  // always with a single-page root. CD.T0Sz encodes the input width,
  // and CD.IPS is driven off IDR5.OAS.
  //
  ...
  Status = SmmuV3SetTranslationStartingLevel (SmmuInfo, InputAddressWidth);
  if (EFI_ERROR (Status)) {
  ...
  // Translation-table walk attributes are always Inner+Outer WBWA +
  // Inner shareable in this driver.
...
/**
  ...
  (Non-secure EL1&0). That pairs with CMD_TLBI_NH_ASID, which flushes
  the NH partition for the given ASID. STRW = 0 is also universally
  implementable (independent of IDR0.HYP / HTTU) and matches how bare-metal /
  hypervisor operating systems re-program Stage 1 STEs after ExitBootServices.
  ...
  )
{
  ...
  // Only meaningful when both stall and terminate are supported (StallModel=0b00);
  // strict IPs may C_BAD_STE if we set this while stalls aren't optional.
  if (Idr0.Bits.StallModel == 0) {
    StreamEntry->Bits.S1StallD = SMMUV3_STREAM_TABLE_ENTRY_S1STALLD_TERMINATE;
  }

  // Memory attributes for CD / Stage 1 translation-table walks:
  // Inner+Outer WBWA + Inner shareable.
...
/**
  ...
  For the shared-ABORT L2 split-on-write path in 2-level stream tables,
  the caller passes a page-sized NewL2 and receives back NewL2Consumed,
  indicating whether it was installed by SmmuV3SplitL1IfShared.
  ...
  )
{
  ...
  *TwoLevelStreamTable = (SmmuInfo->StreamTableEntryMax >= (EFI_PAGE_SIZE / sizeof (SMMUV3_STREAM_TABLE_ENTRY)));
  ...
  // S1ContextPtr = 0 (no CD yet) and Valid = 0, so any DMA from a non-promoted
  // StreamID will trigger a SMMU fault and be recorded in
  // the event queue. The first IoMmu Map/SetAttribute for a StreamID
  // publishes a real CD pointer in-place via SmmuV3PromoteSteToStage1Translate().
  ...
  )
{
  ...
  // invalidations (CMD_TLBI_*) are completed (SMMUv3 spec section 4.6.3).
  SMMUV3_BUILD_CMD_SYNC_NO_INTERRUPT (&Command);
  Status = SmmuV3SendCommand (SmmuInfo, &Command);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Error sending command.\n", __func__));
...
/**
  ...
  At init time every STE is built in "abort-equivalent" mode (Stage 1 translate
  with S1ContextPtr=0 and Valid=0), so no global page-table root is needed;
  per-StreamID roots and CDs are allocated lazily on the first IoMmu map.
  ...
  )
{
  ...
  // Cache ASID width and seed the allocator. ASID 0 is
...
/**
  Free every per-StreamID Stage 1 page-table root and Context Descriptor
  reachable from this SMMU's stream table.

  The STEs and their CDs are the single source of truth for what roots exist.
  Multiple StreamIDs can alias the same CD and root (a device's secondary
  StreamIDs share its primary's CD / ASID), so this walks every STE and
  dedupes the CD and Ttb0 addresses it finds. Each unique root is freed
  exactly once via SmmuV3FreePageTableTree, and each unique CD via
  SmmuV3FreeContextDescriptor.

  If either dedupe-tracking buffer cannot be allocated, the function leaks
  the per-stream roots and CDs rather than risking a double free.
  ...
  )
{
  ...
  // per-stream allocator (each call hands out one ASID before
  ...
  // allocate both dedupe buffers, leak rather than risk double-free.
  if ((MaxRoots == 0) || ((FreedRoots != NULL) && (FreedCds != NULL))) {
    MaxStreamId = SmmuInfo->StreamTableEntryMax;
...
/**
  ...
  Must run after SmmuV3FreePerStreamPageTableRoots so each STE remains
  available while its CD and page-table root are being reclaimed.
...
/**
  ...
  Configures IORT, and SMMUv3 hardware for Stage 1 translation
  based on the configuration data from gSmmuConfigHobGuid HOB.
  ...
  )
{
  ...
  mIortData = IortData;
  mIortSize = SmmuConfig->IortSize;

  Status = SmmuV3InstallOrDeferIortTable (IortData, SmmuConfig->IortSize);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to install/defer IORT table\n", __func__));
    goto Error;
  }
  ...
  for (SmmuIndex = 0; SmmuIndex < mIoMmu->SmmuCount; SmmuIndex++) {
    if (mIoMmu->SmmuInfo[SmmuIndex].Enabled) {
      ...
      DEBUG ((
        DEBUG_INFO,
        "%a: SMMUv3 0x%llx is configured for Stage1 Translation with starting level 0x%llx\n",
        __func__,
        mIoMmu->SmmuInfo[SmmuIndex].SmmuBase,
        mIoMmu->SmmuInfo[SmmuIndex].TranslationStartingLevel
        ));

      // Pre-map any IORT RMR ranges into the per-StreamID Stage 1 page tables that the RMR's IdMappings cover.
