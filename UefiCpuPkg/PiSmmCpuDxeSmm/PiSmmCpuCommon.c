  ...
  )
{
  ...
  ASSERT (*NumberOfCpus <= FixedPcdGet32 (PcdCpuMaxLogicalProcessorNumber));

  //
  // If support CPU hot plug, we need to allocate resources for possibly hot-added processors
  //
  if (FeaturePcdGet (PcdCpuHotPlugSupport)) {
    *MaxNumberOfCpus = FixedPcdGet32 (PcdCpuMaxLogicalProcessorNumber);
  } else {
  ...
  )
{
  ...
  //
  mSmmCodeAccessCheckEnable = FixedPcdGetBool (PcdCpuSmmCodeAccessCheckEnable);
  ...
  DEBUG ((DEBUG_INFO, "PcdControlFlowEnforcementPropertyMask = %d\n", FixedPcdGet32 (PcdControlFlowEnforcementPropertyMask)));
  if (FixedPcdGet32 (PcdControlFlowEnforcementPropertyMask) != 0) {
  ...
  //
  mSmmStackSize = EFI_PAGES_TO_SIZE (EFI_SIZE_TO_PAGES (FixedPcdGet32 (PcdCpuSmmStackSize)));
  ...
  if ((FixedPcdGet32 (PcdControlFlowEnforcementPropertyMask) != 0) && mCetSupported) {
    mSmmShadowStackSize = EFI_PAGES_TO_SIZE (EFI_SIZE_TO_PAGES (FixedPcdGet32 (PcdCpuSmmShadowStackSize)));
