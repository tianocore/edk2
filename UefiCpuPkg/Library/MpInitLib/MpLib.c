  ...
  )
{
  if (SizeBelow1Mb != NULL) {
    *SizeBelow1Mb = ALIGN_VALUE (AddressMap->ModeTransitionOffset, 8) + sizeof (MP_CPU_EXCHANGE_INFO);
  }
  ...
  )
{
  ...
  if (CpuMpData->WakeupBuffer == (UINTN)-1) {
    ...
    //
    // Align MpCpuExchangeInfo to avoid split-locks
    //
    CpuMpData->MpCpuExchangeInfo = (MP_CPU_EXCHANGE_INFO *)(UINTN)
                                   (CpuMpData->WakeupBuffer +
                                    ALIGN_VALUE (CpuMpData->AddressMap.ModeTransitionOffset, 8));
    DEBUG ((
      DEBUG_INFO,
