...
/**
  Save CpuData pointer for current AP.

  @param[in] CpuData    The pointer to CPU_AP_DATA of current AP.
**/
STATIC
VOID
ApSaveCpuData (
...
{
  __asm__ volatile ("mv tp, %0" : : "r"(CpuData) :);
}

/**
  Get CpuData pointer for current AP.

  @return The pointer to CPU AP Data structure for current AP.
**/
STATIC
CPU_AP_DATA *
ApGetCpuData (
  ...
  )
{
  ...
  // Save CpuData pointer for current AP.
  //
  ApSaveCpuData (CpuData);

  //
  // RISC-V scratch register is used to save PeiServicePointer, so use its value of
  // BSP for all APs.
  //
  if (CpuData->ProcessorNumber == CpuMpData->BspNumber) {
    CpuData->ScratchRegValue = RiscVGetSupervisorScratch ();
  } else {
    CpuData->ScratchRegValue = CpuMpData->CpuData[CpuMpData->BspNumber].ScratchRegValue;
  ...
  )
{
  ...
  if ((Hob == NULL) || (GET_GUID_HOB_DATA_SIZE (Hob) != sizeof (UINT64))) {
    DEBUG ((DEBUG_ERROR, "[%a] - Failed to retrieve FDT HOB.\n", __func__));
    return;
  }
  ...
  if ((Hob == NULL) || (GET_GUID_HOB_DATA_SIZE (Hob) != sizeof (RISCV_SEC_HANDOFF_DATA))) {
    DEBUG ((DEBUG_ERROR, "[%a] - Failed to retrieve SEC_HANDOFF_DATA HOB.\n", __func__));
    return;
  }
  ...
  if (Node < 0) {
    DEBUG ((DEBUG_ERROR, "[%a] - Failed to find /cpus node in FDT.\n", __func__));
    return;
  }
  ...
  Index                                           = 0;
  while (Node >= 0 && CpuMpData->CpuCount < PcdGet32 (PcdCpuMaxLogicalProcessorNumber)) {
    Prop = FdtGetProp (FdtBase, Node, "status", &Len);
    if ((Prop != NULL) && (Len == 5) && (AsciiStrnCmp ((CHAR8 *)Prop, "okay", 4) == 0)) {
      ...
      if (Prop != NULL) {
        if (Len == sizeof (UINT32)) {
          ...
          DEBUG ((DEBUG_ERROR, "[%a] - Failed to get HartId from FDT.\n", __func__));
          return;
        }
        ...
        if (HartId != SecData->BootHartId) {
          CpuMpData->CpuData[Index++].HartId = HartId;
          CpuMpData->CpuCount++;
        }
      }
    }
  ...
  )
{
  ...
  if (CpuMpData->FinishedCount != CpuMpData->CpuCount - 1) {
    DEBUG ((DEBUG_ERROR, "[%a] - Only %d APs are initialized successfully.\n", __func__, CpuMpData->FinishedCount));
    return EFI_TIMEOUT;
  }
  ...
  if (Broadcast) {
    for (Index = 0; Index < CpuMpData->CpuCount; Index++) {
      if (Index != CpuMpData->BspNumber) {
        ...
        if (CpuData->Disabled && !WakeUpDisabledAps) {
          continue;
        }

        CpuData->Disabled           = FALSE;
  ...
  )
{
  ...
  //
  if (CpuMpData->CpuData[ProcessorNumber].Disabled) {
    return EFI_INVALID_PARAMETER;
  }
  ...
  )
{
  ...
  if (HealthData != NULL) {
    HealthData->Uint32 = PROCESSOR_HEALTH_STATUS_BIT;
  }
  ...
  )
{
  ...
  CpuData = ApGetCpuData ();
  ASSERT (CpuData != NULL);

  *ProcessorNumber = CpuData->ProcessorNumber;
  ...
  )
{
  ...
  for (Index = 0; Index < ProcessorNumber; Index++) {
    if (!CpuMpData->CpuData[Index].Disabled) {
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "[%a] - Failed to get processor number.\n", __func__));
    return Status;
  }
  ...
  if (!EnableAP) {
    CpuMpData->CpuData[ProcessorNumber].Disabled = TRUE;
  } else {
    CpuMpData->CpuData[ProcessorNumber].Disabled = FALSE;
  }
  ...
  )
{
  ...
  for (Index = 0; Index < MaxLogicalProcessorNumber; Index++) {
    ...
    CpuMpData->CpuData[Index].Disabled  = FALSE;
    if (Index != CpuMpData->BspNumber) {
