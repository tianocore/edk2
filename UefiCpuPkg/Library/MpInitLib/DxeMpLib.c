  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    if ((MemDesc.Capabilities & EFI_MEMORY_XP) == EFI_MEMORY_XP ) {
      if ((MemDesc.Attributes & EFI_MEMORY_XP) == EFI_MEMORY_XP) {
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a - Removing Nx on 0x%p returned %r\n", __func__, BaseAddress, Status));
    ASSERT_EFI_ERROR (Status);
  }
}
  ...
  )
{
  ...
  Status = gDS->GetMemorySpaceDescriptor (BaseAddress, &MemDesc);
  if (!EFI_ERROR (Status)) {
    if ((MemDesc.Capabilities & EFI_MEMORY_RO) == EFI_MEMORY_RO ) {
      if ((MemDesc.Attributes & EFI_MEMORY_RO) != EFI_MEMORY_RO) {
        Status = gDS->SetMemorySpaceAttributes (
                        BaseAddress,
                        Length,
