...
EFI_STATUS cxl_find_regblock_instance(CXL_CONTROLLER_PRIVATE_DATA *Private, enum cxl_regloc_type type, UINT32 index)
{
  ...
  regloc = pci_find_dvsec_capability(Private, CXL_PCI_DVSEC_VENDOR_ID, CXL_DVSEC_ID_REGISTER_LOCATOR);
  if (regloc == 0) {
...
EFI_STATUS cxl_pci_mbox_send(CXL_CONTROLLER_PRIVATE_DATA *Private)
{
  ...
  */
  ...
  if (out_len && Private->mbox_cmd.payload_out) {
    size_t n = minimumOfThree(Private->mbox_cmd.size_out, Private->mds.payload_size, out_len);
    ...
    n = minimumOfThree(Private->mbox_cmd.size_out, Private->mds.payload_size, out_len);
...
EFI_STATUS cxl_pci_setup_mailbox(CXL_CONTROLLER_PRIVATE_DATA *Private)
{
  ...
  Private->mds.payload_size = minimumOfTwo(Private->mds.payload_size, CXL_SZ_1M);
