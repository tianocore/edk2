...
/**
  Determine whether the default UC policy should be skipped for a PCI memory
  BAR, based on an optional platform-supplied protocol. If no such protocol
  is published, the legacy conservative behavior (UC) is preserved.
  ...
  )
{
  ...
  return PciHostMemoryPolicy->ShouldSkipDefaultUcPolicy (PciHostMemoryPolicy, PciIo, BarIndex);
}
...
/**
  ...
  the platform policy must not have requested that the default be skipped.
  ...
  @retval FALSE  The BAR should be left without a firmware cache attribute,
                 either because it is not a memory BAR or because the
                 platform policy exempted it.
  ...
  )
{
  ...
  return !ShouldSkipDefaultUcPolicy (PciIo, BarIndex);
}
