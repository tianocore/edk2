  ...
  )
{
  ...
  //
  // Section "6.30.2 Operation" of PCIe spec states that
  // a DOE instance must complete processing a received object
  // and generate a response and set DOR within 1 second.
  //
  // Timeout is in microseconds
  //
  Timeout = 1000000;
  ...
  )
{
  ...
  do {
    ...
    Count  = 3 * 4;
    Status = PciIoDoeSend (This, Count, &Packet.Uint32);
    if (EFI_ERROR (Status)) {
      return EFI_UNSUPPORTED;
    }
    ...
    ResponseType = (Response[0] >> 16) & 0xFFFF;
