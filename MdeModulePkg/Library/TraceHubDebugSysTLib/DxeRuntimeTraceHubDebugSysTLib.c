...
GLOBAL_REMOVE_IF_UNREFERENCED TRACEHUB_DEBUG_INFO_HOB  *mThDebugInstArrayRt               = NULL;
GLOBAL_REMOVE_IF_UNREFERENCED UINT32                   mDbgInstCountRt                    = 0;
GLOBAL_REMOVE_IF_UNREFERENCED EFI_EVENT                mTraceHubVirtualAddressChangeEvent = NULL;
...
VOID
EFIAPI
TraceHubVirtualAddressChangeEvent (
  IN  EFI_EVENT  Event,
  IN  VOID       *Context
  )
{
  if (gRT != NULL) {
    gRT->ConvertPointer (EFI_OPTIONAL_PTR, (VOID **)&mThDebugInstArrayRt);
    gRT->ConvertPointer (EFI_OPTIONAL_PTR, (VOID **)&mThDebugInstArrayRt->TraceAddress);
    gRT->ConvertPointer (EFI_OPTIONAL_PTR, (VOID **)&mDbgInstCountRt);
  }
...
{
  if (mTraceHubVirtualAddressChangeEvent == NULL) {
    if (gBS != NULL) {
      gBS->CreateEventEx (
             EVT_NOTIFY_SIGNAL,
             TPL_NOTIFY,
             ...
             &mTraceHubVirtualAddressChangeEvent
             );
    }
  }
}
  ...
  )
{
  ...
  for (Index = 0; Index < DbgInstCount; Index++) {
    Status = CheckWhetherToOutputMsg (
               &MipiSystHandle,
               (UINT8 *)&mThDebugInstArrayRt[Index],
  ...
  )
{
  ...
  if (mThDebugInstArrayRt == NULL) {
    ...
    mDbgInstCountRt = DbgInstCount;
    if (mDbgInstCountRt != 0) {
      mThDebugInstArrayRt = AllocateRuntimeZeroPool (sizeof (TRACEHUB_DEBUG_INFO_HOB) * mDbgInstCountRt);
      if (mThDebugInstArrayRt == NULL) {
      ...
      if (DgbContext != NULL) {
        for (Index = 0; Index < mDbgInstCountRt; Index++) {
          CopyMem (&mThDebugInstArrayRt[Index], GET_GUID_HOB_DATA (DgbContext), sizeof (TRACEHUB_DEBUG_INFO_HOB));
          DgbContext = GetNextGuidHob (&gTraceHubDebugInfoHobGuid, GET_NEXT_HOB (DgbContext));
        }
      } else {
        for (Index = 0; Index < mDbgInstCountRt; Index++) {
          mThDebugInstArrayRt[Index].TraceAddress = FixedPcdGet64 (PcdTraceHubDebugAddress);
          mThDebugInstArrayRt[Index].Flag         = FixedPcdGetBool (PcdEnableTraceHubDebugMsg);
          mThDebugInstArrayRt[Index].DebugLevel   = FixedPcdGet8 (PcdTraceHubDebugLevel);
        }
      }
    } else {
      return RETURN_ABORTED;
