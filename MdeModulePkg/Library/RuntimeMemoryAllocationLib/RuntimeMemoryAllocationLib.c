...
#define MIN_REQUIRED_BLOCKS  PcdGet32 (PcdRuntimeMemoryPageCount)
#define SCRATCH_BUFFER_SIZE  (MIN_REQUIRED_BLOCKS * EFI_PAGE_SIZE)
...
//
STATIC RT_MEMORY_PAGE_TABLE  *mRTPageTable = NULL;
...
VOID
EFIAPI
RuntimeMemAllocLibAddressChangeEvent (
  IN  EFI_EVENT  Event,
  IN  VOID       *Context
  )
{
  ...
  //
  gRT->ConvertPointer (0x0, (VOID **)&mRTPageTable->DataAreaBase);
  gRT->ConvertPointer (0x0, (VOID **)&mRTPageTable);
}
  ...
  )
{
  ...
  if (ScratchBufferSize < SCRATCH_BUFFER_SIZE) {
  ...
  )
{
  ...
  if (OldPtr == NULL) {
    return RuntimeAllocateMem (Size);
  }

  if (((UINTN)OldPtr < (UINTN)mRTPageTable->DataAreaBase) ||
      ((UINTN)OldPtr > (((UINTN)mRTPageTable) + SCRATCH_BUFFER_SIZE)))
  {
    return NULL;
  }
  ...
  //
  StartOffset    = (UINTN)OldPtr - (UINTN)mRTPageTable->DataAreaBase;
  ...
  NewPtr = RuntimeAllocateMem (Size);
  if (NewPtr == NULL) {
    return NULL;
  }

  CopyMem (NewPtr, OldPtr, RT_PAGES_TO_SIZE (PageCount));

  RuntimeFreeMem (OldPtr);
  ...
  )
{
  ...
  if ((Buffer == NULL) || (mRTPageTable == NULL)) {
    return;
  }
...
EFI_STATUS
EFIAPI
RuntimeMemoryAllocationLibConstructor (
  ...
  )
{
  ...
  //
  Buffer = AllocateRuntimePool (SCRATCH_BUFFER_SIZE);
  Status = InitializeScratchMemory (Buffer, SCRATCH_BUFFER_SIZE);
  if (EFI_ERROR (Status)) {
    return Status;
  }
