...
- Configure PEI core to be shadowed to memory via
  [PcdShadowPeimOnBoot](https://github.com/tianocore/edk2/blob/HEAD/MdeModulePkg/MdeModulePkg.dec#L1402C34-L1402C53).
...
are initialized post-mem, PEI core will first check if it has been shadowed to memory because the memory bins depend
on a writeable global variable. If PEI core has not been shadowed to memory, PEI memory bins will not be enabled and
the memory bin flow will try again in DXE.

Next, PEI core will attempt to locate the Install PEI Memory Bins PPI, which is the opt in mechanism to PEI memory bins.
If the PPI is not discoverd, the memory bin flow will try again in DXE.
...
flowchart TD
    ...
    PEICoreRelaunch --> CheckShadowed{PEI Core Shadowed & PPI Produced?}

    CheckShadowed -->|No| DXEOnly[DXE only bins]
    CheckShadowed -->|Yes| InitPEIBins[Initialize PEI Memory Bins]
