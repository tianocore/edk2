...
EFI_STATUS
EFIAPI
VarCheckHiiLibRegisterMmHandler (
  ...
  )
{
  ...
  if (mVarCheckHiiBinMmSizeRecieved < sizeof (VAR_CHECK_HII_VARIABLE_HEADER)) {
    DEBUG ((DEBUG_ERROR, "VarCheckHiiLibRegisterMmHandler - MM Communication buffer size is invalid for this handler!\n"));
  ...
  )
{
  ...
  Status = gMmst->MmiHandlerRegister (VarCheckHiiLibRegisterMmHandler, &gEfiVariableCheckHiiCommunicationGuid, &DispatchHandle);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "VarCheckLibHiiRegisterMmHandler - Failed to register handler - %r!\n", Status));
