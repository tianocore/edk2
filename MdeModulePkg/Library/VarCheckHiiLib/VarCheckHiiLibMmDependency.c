  ...
  )
{
  ...
  VarCheckHiiGen (&mVarCheckHiiBinMmSend, &mVarCheckHiiBinMmSendSize);
  if ((mVarCheckHiiBinMmSendSize == 0) || (mVarCheckHiiBinMmSend == NULL)) {
  ...
  for (Index = 0; Index < PiSmmCommunicationRegionTable->NumberOfEntries; Index++) {
    if (MmCommMemRegion->Type == EfiConventionalMemory) {
      ...
      if (CommBufferSize >= (sizeof (EFI_MM_COMMUNICATE_HEADER) + mVarCheckHiiBinMmSendSize)) {
        break;
      }
    }
  ...
  CommBufferSize = OFFSET_OF (EFI_MM_COMMUNICATE_HEADER, Data) + mVarCheckHiiBinMmSendSize;
  ZeroMem (CommHeader, CommBufferSize);
  CopyGuid (&CommHeader->HeaderGuid, &gEfiVariableCheckHiiCommunicationGuid);
  CommHeader->MessageLength = mVarCheckHiiBinMmSendSize;
  VarCheckHiiVariable       = (VAR_CHECK_HII_VARIABLE_HEADER *)(CommHeader->Data);
  CopyMem (VarCheckHiiVariable, mVarCheckHiiBinMmSend, mVarCheckHiiBinMmSendSize); // seperate
