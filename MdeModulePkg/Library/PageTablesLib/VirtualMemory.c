...
BOOLEAN  mPcdSetNxForStack                    = FALSE;
BOOLEAN  mPcdIa32EferChangeAllowed            = FALSE;
BOOLEAN  mPcdCpuStackGuard                    = FALSE;
BOOLEAN  mPcdUse1GPageTable                   = FALSE;
BOOLEAN  mPcdUse5LevelPageTable               = FALSE;
UINT8    mPcdNullPointerDetectionPropertyMask = 0;
UINT32   mPcdImageProtectionPolicy            = 0;
UINT64   mPcdDxeNxMemoryProtectionPolicy      = 0;
UINT64   mPcdPteMemoryEncryptionAddressOrMask = 0;
UINT64   mPgTableMask                         = 0;
  ...
  )
{
  ...
  mPcdSetNxForStack                    = Settings->PcdSetNxForStack;
  mPcdIa32EferChangeAllowed            = Settings->PcdIa32EferChangeAllowed;
  mPcdCpuStackGuard                    = Settings->PcdCpuStackGuard;
  mPcdUse1GPageTable                   = Settings->PcdUse1GPageTable;
  mPcdUse5LevelPageTable               = Settings->PcdUse5LevelPageTable;
  mPcdNullPointerDetectionPropertyMask = Settings->PcdNullPointerDetectionPropertyMask;
  mPcdImageProtectionPolicy            = Settings->PcdImageProtectionPolicy;
  mPcdDxeNxMemoryProtectionPolicy      = Settings->PcdDxeNxMemoryProtectionPolicy;
  mPcdPteMemoryEncryptionAddressOrMask = Settings->PcdPteMemoryEncryptionAddressOrMask;
  mPgTableMask                         = Settings->PgTableMask;
...
{
  return (mPcdNullPointerDetectionPropertyMask & BIT0) != 0;
}
  ...
  )
{
  ...
  //
  return (mPcdSetNxForStack ||
          mPcdDxeNxMemoryProtectionPolicy != 0 ||
          mPcdImageProtectionPolicy != 0);
}
  ...
  )
{
  ...
  if (mPcdCpuStackGuard) {
    if ((StackBase >= Address) && (StackBase < (Address + Size))) {
      return TRUE;
    }
  }

  if (mPcdSetNxForStack) {
  ...
  )
{
  ...
  //
  AddressEncMask = mPcdPteMemoryEncryptionAddressOrMask & PAGING_1G_ADDRESS_MASK_64;
  ...
  *PageEntry2M = (UINT64)(UINTN)PageTableEntry | AddressEncMask | IA32_PG_P | IA32_PG_RW;
  ...
  for (IndexOfPageTableEntries = 0; IndexOfPageTableEntries < 512; IndexOfPageTableEntries++, PageTableEntry++, PhysicalAddress4K += SIZE_4KB) {
    ...
    if ((IsNullDetectionEnabled () && (PhysicalAddress4K == 0)) ||
        (mPcdCpuStackGuard && (PhysicalAddress4K == StackBase)))
    {
  ...
  )
{
  ...
  AddressEncMask = mPcdPteMemoryEncryptionAddressOrMask &
  ...
  )
{
  ...
  PageMapLevel4Entry  = (VOID *)(Cr3BaseAddress & ~mPgTableMask);
  ...
  while (Length > 0) {
    ...
    if (PageDirectory1GEntry->Bits.MustBe1) {
      ...
      if (((PhysicalAddress & (BIT30 - 1)) == 0) && (Length >= BIT30)) {
        if (Hook != NULL) {
          Hook ((UINT64)(UINTN)&PageDirectory1GEntry->Uint64, PhysicalAddress, BIT30, 0);
      ...
      if (PageDirectory2MEntry->Bits.MustBe1) {
        ...
        if (((PhysicalAddress & (BIT21-1)) == 0) && (Length >= BIT21)) {
          if (Hook != NULL) {
            Hook ((UINT64)(UINTN)&PageDirectory2MEntry->Uint64, PhysicalAddress, BIT21, 0);
        ...
        if (Hook != NULL) {
          ...
          Hook ((UINT64)(UINTN)&PageTableEntry->Uint64, PhysicalAddress, EFI_PAGE_SIZE, 0);
        }
