...
/**
  ...
  GCD memory space map and adds every overlapping
  EfiGcdMemoryTypeNonExistent descriptor as
  EfiGcdMemoryTypeMemoryMappedIo with the requested attributes. Existing
  EfiGcdMemoryTypeMemoryMappedIo descriptors are accepted only when their
  capabilities contain all requested attributes. Existing descriptors of any
  other type, or MMIO descriptors without the requested attributes, are treated
  as conflicts.

  This function only ensures that MMIO GCD descriptors exist. It does not set
  memory space attributes or perform architecture-specific attribute
  configuration.
  ...
  )
{
  ...
  DEBUG ((
    DEBUG_INFO,
    "Map Memory: 0x%08lx - 0x%08lx : 0x%08lx\n",
    Base,
    Length,
    Attributes
    ));
