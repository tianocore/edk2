  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    Print (L"Failed to create Wait Event\n");
    return Status;
  }
  ...
  if (EFI_ERROR (Status)) {
    ...
    Status = gBS->WaitForEvent (1, &WaitEvent, &Index);

    if ((FailedCpuList != NULL) && (FailedCpuList[0] != END_OF_CPU_LIST)) {
      Print (L"Printing list of failed CPUs:\n");
      while (FailedCpuList[Index] != END_OF_CPU_LIST) {
        Print (L"%u ", FailedCpuList[Index]);
        Index++;
      }
