/** @file
  ...
  Shared logic between cores to work with memory bins for S4 resume stability. This code was requested to be duplicated
  between PEI Core and DXE Core until a BaseTools feature comes online to support recommended library instances. Any
  changes to this file must also be made in MdeModulePkg/Core/Dxe/Mem/MemoryBin.c.
  ...
  )
{
  ...
  Length = LShiftU64 (NumberOfPages, EFI_PAGE_SHIFT);
