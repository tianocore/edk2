/** @file
  Depex debug helper for SMM Core.

  This file provides diagnostic capabilities for SMM driver dependency
...
/**
  Build a cache of driver state information from the SMM core's discovered list.
  ...
  )
{
  ...
  //
  // Try SMM protocols first
  //
  Status = SmmLocateProtocol (ProtocolGuid, NULL, &Interface);
  if (!EFI_ERROR (Status)) {
    if (Exists != NULL) {
      *Exists = TRUE;
    }
  ...
  //
  // Also check UEFI protocols (SMM drivers can depend on UEFI protocols)
  //
  ...
  )
{
  ...
  for (Index = 0; Index < mDriverStateCount; Index++) {
    if (CompareGuid (&mDriverStateCache[Index].FileNameGuid, CurrentDriver)) {
      for (Link = mDiscoveredList.ForwardLink; Link != &mDiscoveredList; Link = Link->ForwardLink) {
        Entry = CR (Link, EFI_SMM_DRIVER_ENTRY, Link, EFI_SMM_DRIVER_ENTRY_SIGNATURE);
