  ...
  )
{
  ...
  if (gMemoryAttributeProtocol != NULL) {
    ...
    // then we leak the memory and return success. This is to avoid a known bootloader bug where it free memory that is
    // marked RO, which can crash the core if DebugClearMemory is enabled or can be passed out to a driver in the next
    // AllocatePages() call, which can cause a crash later on.
