    ...
    for (Type = (EFI_MEMORY_TYPE)0; Type < EfiMaxMemoryType; Type++) {
      ...
      if (mMemoryTypeStatistics.Statistics[Type].Special &&
          ...
          MemoryRegionsIntersect (
            EntryStart,
            EntryEnd,
            mMemoryTypeStatistics.Statistics[Type].BaseAddress,
            mMemoryTypeStatistics.Statistics[Type].MaximumAddress
            )
          )
      {
        // Verify that it is completely contained within the bin.
        if ((EntryStart < mMemoryTypeStatistics.Statistics[Type].BaseAddress) ||
            (EntryEnd > mMemoryTypeStatistics.Statistics[Type].MaximumAddress))
        {
          DEBUG ((
            DEBUG_ERROR,
