...
class DscBuildData(PlatformBuildClassObject):
    ...
    def GenerateByteArrayValue (self, StructuredPcds):
        ...
        if sys.platform == "win32":
            MakeApp = MakeApp + 'APPFILE = %s\%s.exe\n' % (self.OutputPath, PcdValueInitName) + 'APPNAME = %s\n' % (PcdValueInitName) + 'OBJECTS = %s\%s.obj %s.obj\n' % (self.OutputPath, PcdValueInitName, os.path.normpath(mws.join(GlobalData.gWorkspace, "BaseTools/Source/C/Common/PcdValueCommon"))) + 'INC = '
        else:
            MakeApp = MakeApp + PcdGccMakefile
            MakeApp = MakeApp + 'APPFILE = %s/%s\n' % (self.OutputPath, PcdValueInitName) + 'APPNAME = %s\n' % (PcdValueInitName) + 'OBJECTS = %s/%s.o %s.o\n' % (self.OutputPath, PcdValueInitName, os.path.normpath(mws.join(GlobalData.gWorkspace, "BaseTools/Source/C/Common/PcdValueCommon"))) + \
        ...
        if sys.platform == "win32":
            ...
            if returncode != 0:
                if self._Toolchain in VsDevCmd:
                    MakeCommand = VsDevCmd[self._Toolchain]
