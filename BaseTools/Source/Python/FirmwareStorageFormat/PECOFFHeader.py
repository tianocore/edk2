...
# # Option Header standard fields PE32
# class EFI_OPTION_HEADER_STANDARD_FIELDS_PE32(Structure):
#     _pack_ = 1
#     _fields_ = [
#         ('Magic',                   c_uint16),           # The unsigned integer that identifies the state of the image file. The most common number is 0x10B, which identifies it as a normal executable file. 0x107 identifies it as a ROM image, and 0x20B identifies it as a PE32+ executable.
#         ('MajorLinkerVersion',      c_uint8),            # The linker major version number.
...
# # Option Header standard fields PE32+
# class EFI_OPTION_HEADER_STANDARD_FIELDS_PE32_PLUS(Structure):
#     _pack_ = 1
#     _fields_ = [
#         ('Magic',                   c_uint16),           # The unsigned integer that identifies the state of the image file. The most common number is 0x10B, which identifies it as a normal executable file. 0x107 identifies it as a ROM image, and 0x20B identifies it as a PE32+ executable.
#         ('MajorLinkerVersion',      c_uint8),            # The linker major version number.
...
# # Option Header windows fields PE32
# class EFI_OPTION_HEADER_WINDOWS_SPECIFIC_FIELDS_PE32(Structure):
#     _pack_ = 1
#     _fields_ = [
#         ('ImageBase',               c_uint32),          # The preferred address of the first byte of image when loaded into memory; must be a multiple of 64 K. 
#                                                         # The default for DLLs is 0x10000000. The default for Windows CE EXEs is 0x00010000. 
...
# # Option Header windows fields PE32+
# class EFI_OPTION_HEADER_WINDOWS_SPECIFIC_FIELDS_PE32_PLUS(Structure):
#     _pack_ = 1
#     _fields_ = [
#         ('ImageBase',               c_uint32),          # The preferred address of the first byte of image when loaded into memory; must be a multiple of 64 K. 
#                                                         # The default for DLLs is 0x10000000. The default for Windows CE EXEs is 0x00010000. 
...
# # Option Header Data directories for both PE32/PE32+
# class EFI_OPTION_HEADER_DATA_DIRECTORIES(Structure):
#     _pack_ = 1
#     _fields_ = [
#         ('Export_Table',            c_uint64),          # The export table address and size. 
#         ('Import_Table',            c_uint64),          # The import table address and size. 
#         ('Resource_Table',          c_uint64),          # The resource table address and size. 
#         ('Exception_Table',         c_uint64),          # The exception table address and size. 
#         ('Certificate_Table',       c_uint64),          # The attribute certificate table address and size. 
#         ('Base_Relocation_Table',   c_uint64),          # The base relocation table address and size.
#         ('Debug',                   c_uint64),          # The debug data starting address and size. 
#         ('Architecture',            c_uint64),          # Reserved, must be 0
#         ('Global_Ptr',              c_uint64),          # The RVA of the value to be stored in the global pointer register. The size member of this structure must be set to zero.
#         ('TLS_Table',               c_uint64),          # The thread local storage (TLS) table address and size. 
#         ('Load_Config_Table',       c_uint64),          # The load configuration table address and size. 
#         ('Bound_Import',            c_uint64),          # The bound import table address and size.
#         ('IAT',                     c_uint64),          # The import address table address and size. 
#         ('Delay_Import_Descriptor', c_uint64),          # The delay import descriptor address and size.
#         ('CLR_Runtime_Header',      c_uint64),          # The CLR runtime header address and size. 
#         ('Reserved',                c_uint64),          # Reserved, must be zero
#     ]


# # Option Header is Image only
# class EFI_OPTION_HEADER_PE32(Structure):
#     _pack_ = 1
#     _fields_ = [
#         ('Standard_fields',          EFI_OPTION_HEADER_STANDARD_FIELDS_PE32),            # The number that identifies the type of target machine. 
#         ('Windows_specific_fields',  EFI_OPTION_HEADER_WINDOWS_SPECIFIC_FIELDS_PE32),    # The number of sections. This indicates the size of the section table, which immediately follows the headers.
#         ('Data_directories',         EFI_OPTION_HEADER_DATA_DIRECTORIES),                # The low 32 bits of the number of seconds since 00:00 January 1, 1970 (a C run-time time_t value), which indicates when the file was created.
#     ]

# class EFI_OPTION_HEADER_PE32_PLUS(Structure):
#     _pack_ = 1
#     _fields_ = [
#         ('Standard_fields',          EFI_OPTION_HEADER_STANDARD_FIELDS_PE32_PLUS),            # The number that identifies the type of target machine. 
#         ('Windows_specific_fields',  EFI_OPTION_HEADER_WINDOWS_SPECIFIC_FIELDS_PE32_PLUS),    # The number of sections. This indicates the size of the section table, which immediately follows the headers.
#         ('Data_directories',         EFI_OPTION_HEADER_DATA_DIRECTORIES),                # The low 32 bits of the number of seconds since 00:00 January 1, 1970 (a C run-time time_t value), which indicates when the file was created.
#     ]
