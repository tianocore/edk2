...
class VfrTree():
    ...
    def GenCFile(self, InputFile, FileName):
        self.__Index = 0
        try:
            with open(FileName, 'w') as f:
                ...
                self.GenCFileDfs(self.__Root, gCFormPkg.PkgLength, f)
                f.write('\n};\n')
            f.close()
        except:
            EdkLogger.error("VfrCompiler", FILE_OPEN_FAILURE, "File open failed for %s" % FileName, None)
    ...
    def GenRecordListFile(self, FileName):
        try:
            with open(FileName, 'w') as f:
                self.GenRecordListFileDfs(self.__Root, f)
            f.close()
        except:
            EdkLogger.error("VfrCompiler", FILE_OPEN_FAILURE, "File open failed for %s" % FileName, None)

    def GenRecordListFileDfs(self, Root, f):
        if Root.OpCode != None:
        ...
        if Root.Child != []:
            for ChildNode in Root.Child:
                self.GenRecordListFileDfs(ChildNode, f)
    ...
    def DumpYamlDfs(self, Root, f):
        ...
        if Root.OpCode != None:
            if Root.OpCode != EFI_IFR_FORM_MAP_OP:
                ...
                FormMap, _ = Root.Data.GetInfo()
                gCFormPkg.PkgLength += FormMap.Header.Length
