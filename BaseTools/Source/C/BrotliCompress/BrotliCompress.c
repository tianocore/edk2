...
static void Version(void) {
  ...
  fprintf(stdout, "BrotliCompress %d.%d.%d\n", major, minor, patch);
}
...
int CompressFile(char *InputFile, uint8_t *InputBuffer, char *OutputFile, uint8_t *OutputBuffer, int Quality, int Gap) {
  ...
  if (InputFileSize > 0) {
    uint32_t SizeHint = InputFileSize < (1 << 30)? (uint32_t)InputFileSize : (1u << 30);
    BrotliEncoderSetParameter(s, BROTLI_PARAM_SIZE_HINT, SizeHint);
  }
...
int DecompressFile(char *InputFile, uint8_t *InputBuffer, char *OutputFile, uint8_t *OutputBuffer, int Quality, int Gap) {
  ...
  BrotliDecoderState* s = BrotliDecoderCreateInstance(BrotliAllocFunc, BrotliFreeFunc, &ScratchBufferSize);
  if (!s) {
    fprintf(stderr, "out of memory\n");
    IsOk = BROTLI_FALSE;
    goto Finish;
  ...
  for (;;) {
    ...
    Result = BrotliDecoderDecompressStream(s, &AvailableIn, &NextIn, &AvailableOut, &NextOut, 0);
  }
Finish:
  if (s) {
...
int main(int argc, char** argv) {
  ...
  if (CompressBool) {
    ...
    if (!Ret) {
      fprintf (stderr, "Compress file[%s] fail\n", InputFile);
      goto Finish;
    }
    //
    ...
    if (strlen(InputFile) + strlen(".tmp") < _MAX_PATH) {
      ...
      fprintf (stderr, "Output file path is too long[%s]\n", OutputFile);
      Ret = BROTLI_FALSE;
      goto Finish;
    }
    memset(Buffer, 0, kFileBufferSize*2);
    ...
    if (!Ret) {
      fprintf (stderr, "Decompress file[%s] fail\n", OutputFile);
      goto Finish;
    }
