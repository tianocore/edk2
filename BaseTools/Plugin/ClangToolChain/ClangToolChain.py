...
class ClangToolChain(IUefiBuildPlugin):
    ...
    def do_pre_build(self, thebuilder):
        ...
        ClangBin = Path(clang_bin_env, "clang.exe" if HostInfo.os == "Windows" else "clang") if clang_bin_env else None
        ...
        if not ClangBin:
            ClangBin = Path(w) if (w := shutil.which("clang")) else None
        # 3. Check Windows LLVM install path.
        if not ClangBin and HostInfo.os == "Windows":
            ClangBin = Path("C:/Program Files/LLVM/bin/clang.exe")
        # 4. Finally try MSVC LLVM install.
        if (not ClangBin or not ClangBin.is_file()) and HostInfo.os == "Windows":
            vs_clang_path = self._configure_windows_vs_host_for_clang(
                HostInfo
            )
            ClangBin = (
                vs_clang_path
        ...
        if ClangBin and ClangBin.is_file():
