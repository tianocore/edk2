    ...
    def do_post_build(self, thebuilder):
        ...
        for arch in thebuilder.env.GetValue("TARGET_ARCH").split():
            ...
            for test in testList:
                #
                # The unit tests are executed in 2 places in Windows. One is in this loop.
                # The other is in the method gen_code_coverage_msvc() that runs each unit test through OpenCppCoverage.
                #
                # The goal of this change is to make sure the call to gen_code_coverage_msvc() is the only time the unit test is executed when code coverage information is being collected.
                # If code coverage information is not being collected then the call to gen_code_coverage_msvc() will not be made and the unit tests must be executed in this loop.
                #
                # Also, if the unit tests are being executed in a Linux env, then gen_code_coverage_msvc() will never be called and OpenCppCoverage will never be called.
                #
                ...
                #
                if (thebuilder.env.GetValue("CODE_COVERAGE") != "TRUE") or (thebuilder.env.GetValue("TOOL_CHAIN_TAG").startswith("GCC") == "FALSE"):
                    continue
