...
///
/// Receive mode for the Fastboot transport.
///
typedef enum {
  GBL_EFI_FASTBOOT_TRANSPORT_SINGLE_PACKET = 0,
  GBL_EFI_FASTBOOT_TRANSPORT_FIXED_LENGTH,
} GBL_EFI_FASTBOOT_RX_MODE;

/// Start the Fastboot transport interface.
typedef
EFI_STATUS
...
/// Stop the Fastboot transport interface.
typedef
EFI_STATUS
...
/// Receive data from the transport interface.
typedef
EFI_STATUS
...
/// Send data through the transport interface.
typedef
EFI_STATUS
...
/// Flush any remaining outgoing data.
typedef
EFI_STATUS
