  ...
  )
{
  ...
  {
    UINT8 const  *SerialEnd   = Cert->serial.p + Cert->serial.len;
    UINT8 const  *SerialStart = Cert->serial.p;
  ...
  )
{
  ...
  RecipientInfosLen = 0;
  for (INTN Idx = (INTN)RecipientCount - 1; Idx >= 0; Idx--) {
    EDKII_ASN1_CHK_ADD (
      RecipientInfosLen,
  ...
  )
{
  ...
  UINT8                        *DerWritePtr;
  UINT32                       DerLen;
  UINTN                        RecipientCount       = 0;
  UINT8                        **RecipientInfoBlobs = NULL;
  UINTN                        *RecipientInfoLens   = NULL;
  ...
  //
  for (mbedtls_x509_crt *Cert = (mbedtls_x509_crt *)X509Stack; Cert != NULL; Cert = Cert->next) {
  ...
  {
    unsigned const  BlockSize = mbedtls_cipher_get_block_size (&CipherCtx);
  ...
  {
    UINTN  Idx = 0;
    for (mbedtls_x509_crt *Cert = (mbedtls_x509_crt *)X509Stack; Cert != NULL; Cert = Cert->next, Idx++) {
      ...
      UINTN const  EncKeyBufSize = mbedtls_pk_get_len (&Cert->pk);
      if (EncKeyBufSize == 0) {
        goto Cleanup;
      }

      UINT8 *const  EncKeyBuf = AllocatePool (EncKeyBufSize);
      if (EncKeyBuf == NULL) {
        goto Cleanup;
      }
      ...
      UINTN  TmpBufSize = Cert->issuer_raw.len + Cert->serial.len + EncKeyBufSize + 256;
      UINT8  *TmpBuf    = AllocatePool (TmpBufSize);
      if (TmpBuf == NULL) {
        FreePool (EncKeyBuf);
        goto Cleanup;
      }

      UINT8  *TmpPtr = TmpBuf + TmpBufSize;
  ...
  //
  for (UINTN I = 1; I < RecipientCount; I++) {
    UINT8  *BlobI = RecipientInfoBlobs[I];
    UINTN  LenI   = RecipientInfoLens[I];
    INTN   J      = (INTN)I - 1;
    while (J >= 0) {
      UINTN  LenJ   = RecipientInfoLens[J];
      UINTN  MinLen = MIN (LenI, LenJ);
      INTN   Cmp    = CompareMem (BlobI, RecipientInfoBlobs[J], MinLen);
      if (Cmp == 0) {
        if (LenI < LenJ) {
          Cmp = -1;
        } else if (LenI > LenJ) {
      ...
      RecipientInfoBlobs[J + 1] = RecipientInfoBlobs[J];
      RecipientInfoLens[J + 1]  = RecipientInfoLens[J];
      J--;
    }

    RecipientInfoBlobs[J + 1] = BlobI;
    RecipientInfoLens[J + 1]  = LenI;
  }
  ...
  for (UINTN Idx = 0; Idx < RecipientCount; Idx++) {
