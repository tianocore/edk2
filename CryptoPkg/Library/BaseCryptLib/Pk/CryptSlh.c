...
/**
 Convert Crypto NID to OpenSSL NID
  ...
  @retval     OpenSSL NID value that corresponds to the Crypto NID value.
...
/**
  ...
  @retval     TRUE if the Crypto NID is recognized and Alg is returned.
...
/**
 Convert Crypto NID to Key Size
  ...
  @retval     TRUE if the Crypto NID is recognized and KeySize is returned.
              FALSE if the Crypto NID is not recognized.
**/
STATIC
BOOLEAN
  ...
  )
{
  ...
  if ((PkeyContext == NULL) || (Data == NULL) || (DataSize == 0) || (Signature == NULL) || (SigSize == 0)) {
    return FALSE;
  }
