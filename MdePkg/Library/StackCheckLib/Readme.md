...
- [StackCheckLib Overview](#stackchecklib-overview)
  ...
  - [Usage](#usage)
    - [Default Stack Check Library Configuration](#default-stack-check-library-configuration)
    - [Custom Stack Check Library Configuration](#custom-stack-check-library-configuration)
      - [Static Stack Check Cookie Configuration](#static-stack-check-cookie-configuration)
      - [Dynamic Stack Cookie Configuration](#dynamic-stack-cookie-configuration)
    - [Disable Stack Check Library](#disable-stack-check-library)
...
by calling rdrand on x86 and RNDR on AARCH64, while on RISCV64, it is generated by polling
the seed CSR introduced in the Zkr extension using the csrrw instruction.  If the random
number generator is not supported on that platform or otherwise returns an error, then the
value will still have the build-time randomized value to fall back on.
...
Note: Currently, RISCV64 does not support Standalone MM Core; therefore, the
StandaloneMmCoreEntryPoint module is not yet supported. Implementation will be considered
