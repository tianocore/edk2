...
#define SBI_MPXY_SHMEM_FLAG_NONE      0x0
#define SBI_MPXY_SHMEM_FLAG_READBACK  0x1
  ...
  )
{
  ...
  UINTN    ShmemPhysHi;
  UINTN    *PrevMemDet;
  UINTN    PrevShmemPhysLo;
  UINTN    PrevShmemPhysHi;

  Flags      = ReadBackOldShmem ? SBI_MPXY_SHMEM_FLAG_READBACK : SBI_MPXY_SHMEM_FLAG_NONE;
  PrevMemDet = NULL;

  if (ShmemVirt == MPXY_INVALID_SHMEM_ADDR) {
    ShmemPhysLo = MAX_UINT32;
    ShmemPhysHi = MAX_UINT32;
  } else {
    ShmemPhysLo = ShmemVirt;
    ShmemPhysHi = 0;
  }
  ...
  if ((Ret.Error != SBI_SUCCESS) || !ReadBackOldShmem || (ShmemVirt == MPXY_INVALID_SHMEM_ADDR)) {
    return TranslateError (Ret.Error);
  }

  if ((PrevShmemPhys == NULL) || (ShmemVirt == 0)) {
    return EFI_INVALID_PARAMETER;
  }

  PrevMemDet      = (UINTN *)ShmemVirt;
  PrevShmemPhysLo = PrevMemDet[0];
  PrevShmemPhysHi = PrevMemDet[1];
  if ((PrevShmemPhysLo == MAX_UINT32) && (PrevShmemPhysHi == MAX_UINT32)) {
    *PrevShmemPhys = MPXY_INVALID_SHMEM_ADDR;
  } else {
    *PrevShmemPhys = (UINTN)(PrevShmemPhysLo);
  }
