...
{
  CONST JEDEC_MANUFACTURERS  *MfgList;
  UINT32                     BankIdx;
  UINT32                     MfgIdx;
  UINT32                     LongestString;
  UINTN                      MfgStrLen;

  LongestString = 0;

  for (BankIdx = 0; BankIdx < JEP106_MANUFACTURERS_NUM_BANKS; BankIdx++) {
    MfgList = Jep106Manufacturers[BankIdx];
    MfgIdx  = 0;

    while (MfgList[MfgIdx].Code != 0xFF) {
      MfgStrLen = AsciiStrLen (MfgList[MfgIdx].Manufacturer);
      if (MfgStrLen > LongestString) {
        LongestString = MfgStrLen;
      }
