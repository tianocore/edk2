...
# Uefi UniversalPayload Format
  |     --Fit        | Binary Format | HandOffPayload - HOB | HandOffPayload - FDT |
  |------------------|---------------|----------------------|----------------------|
  |     True         | FIT           | V                    | V                    |
  |     False        | ELF           | V (Default)          | V (Default)          |
...
# How to build UEFI UniversalPayload
  ...
  - Binary Format - ELF
    ...
    - SpecRevision - 1.0
      ...
      - fdtdump Build/UefiPayloadPkgX64/UniversalPayload.elf

  - Binary Format - FIT
    - SpecRevision - 0.7
      - python UefiPayloadPkg/UniversalPayloadBuild.py -t <TOOL_CHAIN_TAG> -s 0.7 -f
      - llvm-objdump -h Build/UefiPayloadPkgX64/UniversalPayload.fit
    - SpecRevision - 0.9
      - python UefiPayloadPkg/UniversalPayloadBuild.py -t <TOOL_CHAIN_TAG> -s 0.9 -f
      - llvm-objdump -h Build/UefiPayloadPkgX64/UniversalPayload.fit
    - SpecRevision - 1.0
      - python UefiPayloadPkg/UniversalPayloadBuild.py -t <TOOL_CHAIN_TAG> -s 1.0 -f
