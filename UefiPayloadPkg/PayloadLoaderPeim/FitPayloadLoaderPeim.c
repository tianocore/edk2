...
#define ESPI_DEV_NUM  31
#define ESPI_FUN_NUM  0
...
/**
  The wrapper function of LocatePciDevicePpi().
  @retval EFI_SUCCESS          gEdkiiPeiPciDevicePpiGuid is discovered.
  @return EFI_NOT_FOUND        No gEdkiiPeiPciDevicePpiGuid is discovered..
**/
EFI_STATUS
EFIAPI
LocatePciDevicePpi (
  ...
  )
{
  ...
  TempNode = FdtAddSubnode (Fdt, SPINode, TempStr);
  ASSERT (TempNode > 0);
  ...
  if (PciRootBridgeInfo != NULL) {
    for (Index = 0; Index < PciRootBridgeInfo->Count; Index++) {
      ...
      if (Index == 0) {
        ...
        Data16 = CpuToFdt16 (PciData.Device.SubsystemID);
        Status = FdtSetProp (Fdt, GmaNode, "subsystem-id", &Data16, sizeof (UINT16));

        Data16 = CpuToFdt16 (PciData.Device.SubsystemVendorID);
        Status = FdtSetProp (Fdt, GmaNode, "subsystem-vendor-id", &Data16, sizeof (UINT16));

        Data16 = CpuToFdt16 (PciData.Hdr.RevisionID);
        Status = FdtSetProp (Fdt, GmaNode, "revision-id", &Data16, sizeof (UINT16));

        Data16 = CpuToFdt16 (PciData.Hdr.DeviceId);
        Status = FdtSetProp (Fdt, GmaNode, "device-id", &Data16, sizeof (UINT16));

        Data16 = CpuToFdt16 (PciData.Hdr.VendorId);
        Status = FdtSetProp (Fdt, GmaNode, "vendor-id", &Data16, sizeof (UINT16));
      } else if (Index == 1) {
        Status   = AsciiSPrint (eSPIStr, sizeof (eSPIStr), "espi@%X,%X", ESPI_DEV_NUM, ESPI_FUN_NUM);
        ...
        Data16 = CpuToFdt16 (1);
        Status = FdtSetProp (Fdt, eSPINode, "#size-cells", &Data16, sizeof (UINT16));

        Data16 = CpuToFdt16 (2);
        Status = FdtSetProp (Fdt, eSPINode, "#address-cells", &Data16, sizeof (UINT16));
