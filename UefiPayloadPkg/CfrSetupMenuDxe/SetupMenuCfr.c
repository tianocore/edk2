  ...
  )
{
  ...
  if (DependencyId == 0) {
    ASSERT ((DepValues == NULL) && (NumDepValues == 0));

    OpHeader.OpCode = EFI_IFR_TRUE_OP;
    OpHeader.Length = sizeof (EFI_IFR_OP_HEADER);
    OpHeader.Scope = 0;
  ...
  )
{
  ...
  *ProcessedLength += sizeof (CFR_OPTION_FORM);
  ...
  if (Option->flags & CFR_OPTFLAG_SUPPRESS) {
    CfrProduceHiiCondition (StartOpCodeHandle, EFI_IFR_SUPPRESS_IF_OP, 0, NULL, 0);
  }
  if (Option->flags & CFR_OPTFLAG_INACTIVE) {
    CfrProduceHiiCondition (StartOpCodeHandle, EFI_IFR_GRAY_OUT_IF_OP, 0, NULL, 0);
  }
