...
/**
  ...
  @param[in]  SizeCells         #size-cells for serial port node 'reg' property.
**/
VOID
ParseSerialPort (
  IN VOID     *Fdt,
  ...
  )
{
  ...
  for (SubNode = FdtFirstSubnode (Fdt, Node); SubNode >= 0; SubNode = FdtNextSubnode (Fdt, SubNode)) {
    ...
    if (AsciiStrnCmp (NodePtr->Name, "isa", AsciiStrLen ("isa")) == 0) {
      ...
      PropertyPtr = FdtGetProperty (Fdt, SubNode, "#size-cells", &TempLen);
      if ((PropertyPtr != NULL) && (TempLen > 0)) {
        SizeCells = Fdt32ToCpu (*(UINT32 *)PropertyPtr->Data);
      }

      SSubNode = FdtFirstSubnode (Fdt, SubNode);
      ParseSerialPort (Fdt, SSubNode, AddressCells, SizeCells);
    }

    if (AsciiStrnCmp (NodePtr->Name, "serial@", AsciiStrLen ("serial@")) == 0) {
      ParseSerialPort (Fdt, SubNode, AddressCells, SizeCells);
    }
  }
  ...
  )
{
  ...
  PropertyPtr = FdtGetProperty (Fdt, 0, "#size-cells", &TempLen);
  if ((PropertyPtr != NULL) && (TempLen > 0)) {
    RootSizeCells = Fdt32ToCpu (*(UINT32 *)PropertyPtr->Data);
    DEBUG ((DEBUG_INFO, " root #size-cells = 0x%x\n", RootSizeCells));
  }
  ...
  for (Node = FdtNextNode (Fdt, 0, &Depth); Node >= 0; Node = FdtNextNode (Fdt, Node, &Depth)) {
    ...
    switch (NodeType) {
      case SerialPort:
        ParseSerialPort (Fdt, Node, RootAddressCells, RootSizeCells);
        break;
      case ReservedMemory:
