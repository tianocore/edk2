  ...
  )
{
  ...
  //
  Snp = AllocatePool (sizeof (SNP_DRIVER));
  if (Snp == NULL) {
  ...
  Snp->Db  = (VOID *)((UINTN)Address + (UINTN)(EFI_PAGE_SIZE / 2));
  Snp->Cdb = (PXE_CDB *)((UINTN)Address + (UINTN)EFI_PAGE_SIZE - (UINTN)sizeof (PXE_CDB));
