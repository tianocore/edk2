  ...
  )
{
  ...
  Status = ArmFfaLibMsgSendDirectReq2 (mTpmPartId, &gTpm2ServiceFfaGuid, &TpmArgs);
  while (Status == EFI_INTERRUPT_PENDING) {
    // We are assuming vCPU0 of the TPM SP since it is UP.
    Status = ArmFfaLibRun (mTpmPartId, 0x00);
  }
  ...
  )
{
  ...
  mTpmPartId  = TpmPartInfo->PartitionId;

  ArmFfaLibRxRelease (PartId);

  Status = Tpm2FfaCheckInterfaceVersion ();
  if (EFI_ERROR (Status)) {
    goto ErrorHandle2;
  }
