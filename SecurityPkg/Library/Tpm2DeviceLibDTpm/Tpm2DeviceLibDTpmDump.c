  ...
  )
{
  ...
  if (Preamble) {
    DEBUG ((DEBUG_TPM, "%a", Preamble));
  }

  // Dump them bytes.
  for (Index = 0; Index < DebugBufferCount; Index++) {
    DEBUG ((DEBUG_TPM, "%02X ", Buffer[Index]));
  }
  ...
  )
{
  ...
  DEBUG ((DEBUG_TPM, "Command:  %a (0x%08X)\n", GetTpm2CommandString (CommHeader->commandCode), NativeCode));
  DEBUG ((DEBUG_TPM, "Tag:      0x%04X\n", NativeTag));
  DEBUG ((DEBUG_TPM, "Size:     %d (0x%X)\n", NativeSize, NativeSize));
  ...
  )
{
  ...
  DEBUG ((DEBUG_TPM, "=== END TPM COMMAND ===\n\n"));
