  ...
  )
{
  ...
  *DataSize = VarInfo.PlainDataSize;
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_INFO, "%a: %d Exit(). VariableName = %s, VariableGuid = 0x%g, DataSize = 0x%X, Data Buffer = 0x%lX, Status = %r\n",
          __FUNCTION__, __LINE__, VariableName, VariableGuid, *DataSize, Data, Status));
    return Status;
  }
