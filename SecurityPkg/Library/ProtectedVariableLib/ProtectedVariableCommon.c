...
{
  return UpdateVariableDigestData (Context, VarInfo, HmacSha256Update);
}
  ...
  )
{
  ...
  if (Global == NULL || VarInfo == NULL || HmacData == NULL) {
    ASSERT (Global != NULL);
    ASSERT (VarInfo != NULL);
    ASSERT (HmacData != NULL);
  ...
  if (!HmacSha256SetKey (Context, Global->MetaDataHmacKey, sizeof (Global->MetaDataHmacKey))
      ...
      || !HmacSha256Final (Context, HmacData))
  {
    ASSERT (FALSE);
    Status = EFI_ABORTED;
  } else {
...
/**
  ...
  @param[out]     SortMethod    Method used to compare node in list.
  ...
  )
{
  ...
  PerformQuickSort (StoreIndexTable,
                    Count,
