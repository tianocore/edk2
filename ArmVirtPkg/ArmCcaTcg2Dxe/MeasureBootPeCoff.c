...
STATIC UINTN  mTcg2DxeImageSize = 0;

/**
  Reads contents of a PE/COFF image in memory buffer.

  Caution: This function may receive untrusted input.
  PE/COFF image is external input, so this function will make sure
  the PE/COFF image content read is within the image buffer.
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    ...
    DEBUG ((
      DEBUG_ERROR,
      "ArmCcaTcg2Dxe: PeImage invalid. Cannot retrieve image information.\n"
      ));
    goto Finish;
  }
  ...
  //    hashing in PE/COFF Specification 8.0 Appendix A.
  //
  //
  ...
  if (NumberOfRvaAndSizes <= EFI_IMAGE_DIRECTORY_ENTRY_SECURITY) {
    ...
    if (Hdr.Pe32->OptionalHeader.Magic == EFI_IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
      ...
      // Use PE32+ offset.
      //
      HashBase = (UINT8 *)&Hdr.Pe32Plus->OptionalHeader.CheckSum +
                 sizeof (UINT32);
    ...
    //
    // 8.  Skip over the Cert Directory.
    //      (It is sizeof(IMAGE_DATA_DIRECTORY) bytes.)
    // 9.  Hash everything from the end of the Cert Directory to the
    //      end of image header.
    //
    if (Hdr.Pe32->OptionalHeader.Magic == EFI_IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
      //
  ...
  //
  Status = HashCompleteAndExtend (HashHandle, MrIndex, NULL, 0, DigestList);
  if (EFI_ERROR (Status)) {
    goto Finish;
  }
