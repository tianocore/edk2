/** @file
  The Arm CCA IoMMU protocol provides support to allocate, free, map and umap
  DMA buffers for bus masters (e.g PciHostBridge).
  When the execution context is a Realm, the DMA operations must be performed
  on buffers that are shared with the Host.
  Hence the Realm Aperture Management Protocol (RAMP) is used to manage the
  sharing of the DMA buffers or in some cases to bounce the buffers.
  ...
  )
{
  ...
  // Initialize the MAP_INFO structure, except the ApertureRef field
  ...
  )
{
  ...
  // Changes to the UEFI memory map are forbidden when MemoryMapLocked = TRUE.
  // So only free the memory if MemoryMapLocked = FALSE.
  if (!MemoryMapLocked) {
    FreePool (MapInfo);
  }
...
/**
  ...
  @retval EFI_INVALID_PARAMETER Mapping is not a value that was returned by
                                Map() or the Bounce Buffer address and the Host
                                Address does not match for BusMasterCommonBuffer
                                operations.
  ...
  )
{
  ...
  // The only valid memory type is EfiBootServicesData
  ...
  if (EFI_ERROR (Status)) {
    ...
    *HostAddress = (VOID *)(UINTN)BbAddress;
    ...
    if (EFI_ERROR (Status)) {
      ...
      *HostAddress = NULL;
      Status1      = gBS->FreePages (BbAddress, Pages);
...
/**
  ...
  @retval EFI_INVALID_PARAMETER The memory range specified by HostAddress and
                                ...
                                Or the Page count did not match the number of
                                pages that were mapped.
  @retval EFI_NOT_FOUND         The mapping for the allocated buffer was not
                                found.
  ...
  )
{
  ...
  if (MapInfo == NULL) {
    ...
    // Make sure we are freeing up all the pages.
    if (MapInfo->NumberOfPages != Pages) {
      ASSERT (MapInfo->NumberOfPages == Pages);
      return EFI_INVALID_PARAMETER;
    }
  ...
  // Release the common buffer itself.
