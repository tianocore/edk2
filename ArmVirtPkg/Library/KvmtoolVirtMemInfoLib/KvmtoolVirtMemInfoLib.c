...
// Maximum number of platform devices.
#define MAX_PLAT_DEVICE_COUNT  16
...
// Platform devices.
STATIC PLATFORM_DEVICE_INFO  mPlatInfo[MAX_PLAT_DEVICE_COUNT];
  ...
  )
{
  ...
  ZeroMem (&mPlatInfo, sizeof (mPlatInfo));
  DevCount = MAX_PLAT_DEVICE_COUNT;
  Status   = ParsePlatformDeviceFdt (DtbBase, mPlatInfo, &DevCount);
  if (EFI_ERROR (Status)) {
    return;
  }

  if ((DevCount == 0) || (DevCount > MAX_PLAT_DEVICE_COUNT)) {
    return;
  }
  ...
  for (Devices = 0; Devices < DevCount; Devices++) {
    VirtualMemoryTable[++Idx].PhysicalBase = mPlatInfo[Devices].BaseAddress;
    VirtualMemoryTable[Idx].VirtualBase    = mPlatInfo[Devices].BaseAddress;
