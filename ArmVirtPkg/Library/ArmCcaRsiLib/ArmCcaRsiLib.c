  ...
  )
{
  ...
  ArmCallSmc (&SmcCmd);
  Status = RsiCmdStatusToReturnStatus (SmcCmd.Arg0);
  ...
  )
{
  ...
  ArmCallSmc (&SmcCmd);
  return RsiCmdStatusToReturnStatus (SmcCmd.Arg0);
}
  ...
  )
{
  ...
  *UefiImpl    = RSI_IMPL_VERSION;
  ...
  if (Status == RETURN_INVALID_PARAMETER) {
    ...
    // by RsiCmdStatusToReturnStatus(), return the status code as
