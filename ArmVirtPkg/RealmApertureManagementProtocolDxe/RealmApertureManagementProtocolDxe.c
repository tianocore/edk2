  ...
  )
{
  ...
  MemEnd = MemStart + EFI_PAGES_TO_SIZE (Pages) - 1;
       ...
       )
  {
    ...
    ApertureEnd   = ApertureStart + EFI_PAGES_TO_SIZE (ApertureInfo->Pages) - 1;
  ...
  )
{
  ...
  STATIC_ASSERT (
    (EFI_PAGE_SIZE & (ARM_CCA_REALM_GRANULE_SIZE - 1)) == 0,
    "(EFI_PAGE_SIZE & (ARM_CCA_REALM_GRANULE_SIZE - 1)) == 0"
    );
  ...
  while (Index < Pages) {
    ...
    // The MemRangeAddr could reside in middle of a descriptor, so
    // compute the offset from the start and use that to advance
    // the Index and MemRangeAddr.
    Offset        = MemRangeAddr - GcdDescriptor.BaseAddress;
    Index        += EFI_SIZE_TO_PAGES (GcdDescriptor.Length - Offset);
    MemRangeAddr += (GcdDescriptor.Length - Offset);
  } // while
  ...
  Status = ArmCcaRsiSetIpaState (
             ...
             (Pages * EFI_PAGE_SIZE),
             RipasEmpty,
             ARM_CCA_RIPAS_CHANGE_FLAGS_RSI_CHANGE_DESTROYED
             );
  if (RETURN_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
...
error_handler1:
  Status1 = ArmCcaSetMemoryProtectionAttribute (
              Memory,
              EFI_PAGES_TO_SIZE (Pages),
              mIpaWidth,
              FALSE
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    DEBUG ((DEBUG_VERBOSE, "RAMP: Hook system reset to close all apertures.\n"));
  ...
  )
{
  ...
  Status = ArmCcaGetIpaWidth (&mIpaWidth);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
